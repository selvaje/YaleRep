...
class ClusteredRandomForestRegressor(BaseEstimator, RegressorMixin):
    ...
    # Random forest built from group-aware decision trees.
    # Why: keep group bootstrap consistent and explicitly free temps to reduce RAM pressure.
    ...
    def _fit_single_tree(self, X, y, groups, rng_seed):
        ...
        # optional subsample of rows already selected via groups
        ...
        # Fit a single group-aware tree on the masked subset
        ...
        est.fit(X_fit, y_fit, groups=groups_fit)

        train_idx = np.where(mask)[0]

        # --- memory discipline ---
        del X_fit, y_fit, groups_fit
        if self.subsample < 1.0:
    ...
    def fit(self, X, y, chunk_size=None):
        rng = check_random_state(self.random_state)
        # first column is IDr used for grouping
    ...
    def _compute_oob_scores(self, X, y):
        # Expect y to be 2D: (n_samples, n_targets)
        ...
        for i, (est, train_idx) in enumerate(zip(self.estimators_, self.train_indices_list)):
            test_idx = np.setdiff1d(arange_idx, train_idx)
            if test_idx.size > 0:
                p = est.predict(X[test_idx])
                if n_targets == 1:
                    all_preds[i, test_idx, :] = p[:, None]
                else:
                    all_preds[i, test_idx, :] = p
            # free loop temps
            del test_idx
            gc.collect()

        preds = np.nanmean(all_preds, axis=0)   # (n_samples, n_targets)
        self.oob_scores_ = []

        # general OOB (averaged across targets)
        ...
        for j in range(n_targets):
            valid_j = ~np.isnan(preds[:, j])
            if np.any(valid_j):
                if self.oob_metric == 'mse':
                    s = np.mean((y[valid_j, j] - preds[valid_j, j])**2)
                elif self.oob_metric == 'rmse':
                    s = np.sqrt(np.mean((y[valid_j, j] - preds[valid_j, j])**2))
                elif self.oob_metric == 'r2':
                    ss_res = np.sum((y[valid_j, j] - preds[valid_j, j])**2)
                    ss_tot = np.sum((y[valid_j, j] - np.mean(y[valid_j, j]))**2)
        ...
        # --- memory discipline ---
        del all_preds, preds, valid, arange_idx
        gc.collect()
...
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YscorerN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_GaRFC.txt', merge_r, delimiter=' ', fmt=fmt)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YscorerhoN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_GaRFC.txt', merge_rho, delimiter=' ', fmt=fmt)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YscoremaeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_GaRFC.txt', merge_mae, delimiter=' ', fmt=fmt)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YscorekgeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_GaRFC.txt', merge_kge, delimiter=' ', fmt=fmt)
