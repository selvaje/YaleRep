...
# shuff=11 
# head -1 $EXTRACTpy/stationID_x_y_valueALL_predictors_Y.txt >  $EXTRACTpy/stationID_x_y_valueALL_predictors_Ys$shuff.txt 
# head -1 $EXTRACTpy/stationID_x_y_valueALL_predictors_X.txt >  $EXTRACTpy/stationID_x_y_valueALL_predictors_Xs$sufff.txt
...
X_train  = pd.read_csv(rf'./stationID_x_y_valueALL_predictors_Xs1000000.txt', header=0, sep=' ', usecols=lambda column: column not in ['ID','YYYY','MM','lon','lat','Xcoord','Ycoord'])
Y_train  = pd.read_csv(rf'./stationID_x_y_valueALL_predictors_Ys1000000.txt', header=0, sep=' ', usecols=lambda column: column not in ['ID','YYYY','MM','lon','lat','Xcoord','Ycoord'])
...
# Compute OOB error for each Q (QMIN to QMAX) at the group level, ensuring IDraster is retained.
    def compute_oob_error(self, Y_true):
        ...
        for idraster in unique_idrasters:
            ...
            if np.any(mask):
                ...
                errors = [mean_squared_error(Y_true_filtered[col], oob_pred_filtered[:, i]) for i, col in enumerate(Y_true_filtered.columns)]
                oob_errors_list.append([idraster] + errors)
        
        oob_errors = np.array(oob_errors_list)
        overall_oob_error = np.mean(oob_errors[:, 1:], axis=0)  # Ignore IDraster column in mean calculation
...
Y_train_predOOB = RFreg.oob_predictions
...
savetxt(rf'./stationID_x_y_valueALL_predictors_YOOBpredictN{N_EST_S}_{obs_s}obs.txt', RFreg.oob_predictions, delimiter=' ',  header='QMIN Q10 Q20 Q30 Q40 Q50 Q60 Q70 Q80 Q90 QMAX', comments='' , fmt=fmt)

savetxt(rf'./stationID_x_y_valueALL_predictors_YpredictN{N_EST_S}_{obs_s}obs.txt', Y_train_pred , delimiter=' ', header='QMIN Q10 Q20 Q30 Q40 Q50 Q60 Q70 Q80 Q90 QMAX', comments='' , fmt=fmt)
