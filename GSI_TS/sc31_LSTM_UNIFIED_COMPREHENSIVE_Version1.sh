...
###### sbatch /nfs/roberts/pi/pi_ga254/hydro/scripts/GSI_TS/sc31_LSTM_UNIFIED_COMPREHENSIVE.sh
...
# Analyze temporal contiguity for each station
...
def analyze_contiguity(group_y):
    ...
    n_cont_months = n_obs  # Simple count; could be refined for actual month counting
...
print(f"  - Avg observations: {train_stations_df['n_obs'].mean():.0f}")
print(f"  - Avg max gap: {train_stations_df['max_gap_days'].mean():.1f} days")

print(f"\n✓ TESTING STATIONS: {len(test_idrs)}")
if len(test_stations_df) > 0:
    print(f"  - Avg observations: {test_stations_df['n_obs'].mean():.0f}")
    print(f"  - Avg max gap: {test_stations_df['max_gap_days'].mean():.1f} days")

# Create time-series split
X_train = X[X['IDr'].isin(train_idrs)].copy()
Y_train = Y[Y['IDr'].isin(train_idrs)].copy()
X_test = X[X['IDr'].isin(test_idrs)].copy()
Y_test = Y[Y['IDr'].isin(test_idrs)].copy()
...
print(f"\nDataset shapes: Train X {X_train.shape}, Train Y {Y_train.shape}")
...
report_ts.add_content(f'Training stations (≥{MIN_TRAIN_OBS} obs, max gap ≤{MAX_GAP_DAYS} days): {len(train_idrs)}')
...
report_ts.add_section('CONTIGUITY METRICS', level=2)
report_ts.add_dataframe(train_stations_df.head(20))

report_ts.save()
print(f"\n✓ Phase 1 report saved: 01_TIMESERIES_SPLIT_REPORT.txt")
...
print(f"Dynamic: REMOVE {len(dynamic_remove)}: {', '.join(dynamic_remove[:5])}..." if len(dynamic_remove) > 5 else f"Dynamic: REMOVE {len(dynamic_remove)}: {', '.join(dynamic_remove)}")
...
print(f"Spatial variance: KEEP {len(static_2a_keep)} variables")

# Stage 2B: Spatial correlation with quantiles
print("\nStage 2B: Spatial correlation with quantiles...")
    ...
    try:
        ...
        for q_var in ['QMIN', 'Q50', 'QMAX']:
            ...
            if np.sum(mask) > 5:
                ...
                if not np.isnan(rho):
                    results.append({'Variable': var, 'Quantile': q_var, 'ρ': abs(rho)})
    except:
        pass
    return results
...
print(f"Spatial correlation: KEEP {len(static_2b_keep)} variables")
...
print(f"Multicollinearity: REMOVED {len(removed_vars)} variables → {len(static_final)} kept")
...
report_fs.add_section('DYNAMIC VARIABLES', level=2)
report_fs.add_content(f'Selected: {len(dynamic_keep)} variables')
for d in dynamic_keep:
    report_fs.add_content(f'  ✓ {d}', indent=1)

report_fs.add_section('STATIC VARIABLES', level=2)
report_fs.add_content(f'Selected: {len(static_final)} variables (from {len(static_present)} original)')
for s in sorted(static_final)[:20]:
    report_fs.add_content(f'  ✓ {s}', indent=1)

report_fs.save()
print(f"\n✓ Phase 2 report saved: 02_FEATURE_SELECTION_REPORT.txt")
...
# Add derived features to test set
...
# Define LSTM model and training infrastructure
print("\nDefining LSTM model...")
...
print(f"Model: {n_dyn} dynamic + {n_sta} static → 128 LSTM → 256 dense → 11 quantiles")
...
def compute_metrics(Y_true_np, Y_pred_np):
    ...
    nse_vals = [1 - (np.sum((Y_true_np[:, i] - Y_pred_np[:, i])**2) / np.sum((Y_true_np[:, i] - np.mean(Y_true_np[:, i]))**2)) for i in range(11)]
...
for metric in ['r', 'nse', 'kge', 'rmse', 'mae']:
    val, _ = train_metrics[metric]
    report_lstm.add_content(f'  {metric.upper()}: {val:.4f}', indent=1)
...
report_lstm.add_section('Per-Quantile Metrics (Test)', level=2)
