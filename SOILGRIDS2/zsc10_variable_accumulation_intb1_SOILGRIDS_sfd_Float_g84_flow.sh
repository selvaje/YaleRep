...
#SBATCH -o /vast/palmer/scratch/sbsc/ga254/stdout/sc10_SOILGRIDS_sfd_Float_g84.sh.%J.out
#SBATCH -e /vast/palmer/scratch/sbsc/ga254/stderr/sc10_SOILGRIDS_sfd_Float_g84.sh.%J.err
#SBATCH --mem=400G
#SBATCH --job-name=sc10_SOILGRIDS_sfd_Float_g84.sh 
...
# gdal_translate -a_srs EPSG:4326 -r bilinear -ot Float32 -tr 0.000833333333333333333 0.000833333333333333333 -co BIGTIFF=YES -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -co NUM_THREADS=ALL_CPUS -co TILED=YES  -projwin $ulx $uly $lrx $lry $tif $RAM/${tifname}_${box}_var.tif
...
cp $MERITH/flow_sfd/flow_sfd_$box.tif     $RAM/${tifname}_flow_sfd_$box.tif
...
r.external  input=$RAM/${tifname}_flow_sfd_$box.tif   output=flow --overwrite  
...
/home/ga254/.grass8/addons/scripts/r.mapcalc.tiled   'varare_acc_flow = float(varare_acc / flow )'   nprocs=8

export GDAL_CACHEMAX=100G
export GDAL_NUM_THREADS=4

r.out.gdal --o -f -c -m createopt='COMPRESS=DEFLATE,ZLEVEL=9,BIGTIFF=YES' nodata=-9999999 type=Float64   input=varare_acc_flow  output=$SOILGRIDSH/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Float_g84_flow.tif 

EOF
"

source ~/bin/gdal3 &> /dev/null
gdalinfo -mm $SOILGRIDSH/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Float_g84_flow.tif | grep Computed | awk '{ gsub(/[=,]/," " , $0 ); print $3 , $4 }' > $SOILGRIDSH/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Float_g84_flow.mm
