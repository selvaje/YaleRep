...
# Ensure X_train_column_names is an ndarray of strings (it already is in your script)
# get the intersection of vars actually present in the training columns
all_cols = X_train_column_names.astype(str)  # ensure dtype str
...
if len(static_present) == 0:
    ...
    # indices of static vars in the original X matrix
    static_idx = np.where(np.isin(all_cols, static_present))[0]
    X_static_train = X_train_np[:, static_idx]
    X_static_test  = X_test_np[:, static_idx]

    # Run GroupAwareRFECV only on the static features
    print(f'Running GroupAwareRFECV on {len(static_present)} static features...')
    selector.fit(
        X_static_train,
        Y_train_np,
        groups=groups_train,
        X_column_names=static_present
    )

    # support mask relative to the static feature set
    support_mask_static = selector.support_
    # selected static names (in the original static_present order)
    selected_static_names = np.array(static_present)[support_mask_static].tolist()
    print(f'Selected {len(selected_static_names)} static features: {selected_static_names}')
...
# This keeps the rest of your script unchanged as requested.
...
print('Final features (first 30):', X_train_column_names[:30].tolist())
