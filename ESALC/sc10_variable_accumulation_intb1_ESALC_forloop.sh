...
### multiply the 1 to 1000000 to differenziate with 0.0001 
...
sbatch  --export=dir=$dir,year=$year,tifname=$tifname  --job-name=sc11_tiling20d_ESALC_${tifname}.sh    --dependency=afterany:$( squeue -u $USER -o "%.9F %.80j" | grep  sc10_var_acc_intb1_ESALC_forloop_${tifname}.sh | awk '{ printf ("%i:" , $1)} END {gsub(":","") ; print $1 }'  ) /gpfs/gibbs/pi/hydro/hydro/scripts/ESALC/sc11_tiling20d_ESALC.sh
sleep 30
fi 
