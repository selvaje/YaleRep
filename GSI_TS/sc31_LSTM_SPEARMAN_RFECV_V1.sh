...
print("\n" + "="*100)
print("SC31: LSTM WITH SPEARMAN DECORRELATION + RFECV PIPELINE")
print("="*100)
...
# Spearman Decorrelation Threshold
SPEARMAN_STATION_THRESHOLD = 0.90   # Station-level decorrelation
...
# =========================================================================
# CREATE DERIVED DYNAMIC FEATURES (on full dataset)
# =========================================================================
print("\n" + "="*100)
print("CREATING DERIVED DYNAMIC FEATURES")
print("="*100)

# Derived features: divide by accumulation area
acc = X['accumulation'].astype('float32')
acc_safe = acc.replace(0, 1e-10)  # Avoid division by zero

derived_features = []

if 'ppt0' in X.columns:
    X['ppt0_area'] = (X['ppt0'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('ppt0_area')

if 'ppt1' in X.columns:
    X['ppt1_area'] = (X['ppt1'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('ppt1_area')

if 'ppt2' in X.columns:
    X['ppt2_area'] = (X['ppt2'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('ppt2_area')

if 'ppt3' in X.columns:
    X['ppt3_area'] = (X['ppt3'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('ppt3_area')

if 'tmin0' in X.columns:
    X['tmin0_area'] = (X['tmin0'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('tmin0_area')

if 'tmin1' in X.columns:
    X['tmin1_area'] = (X['tmin1'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('tmin1_area')

if 'tmin2' in X.columns:
    X['tmin2_area'] = (X['tmin2'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('tmin2_area')

if 'tmin3' in X.columns:
    X['tmin3_area'] = (X['tmin3'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('tmin3_area')

if 'tmax0' in X.columns:
    X['tmax0_area'] = (X['tmax0'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('tmax0_area')

if 'soil0' in X.columns:
    X['soil0_area'] = (X['soil0'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('soil0_area')

if 'soil1' in X.columns:
    X['soil1_area'] = (X['soil1'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('soil1_area')

if 'soil2' in X.columns:
    X['soil2_area'] = (X['soil2'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('soil2_area')

if 'soil3' in X.columns:
    X['soil3_area'] = (X['soil3'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('soil3_area')

if 'GRWLw' in X.columns:
    X['GRWLw_area'] = (X['GRWLw'].astype('float32') / acc_safe).astype('float32')
    derived_features.append('GRWLw_area')

print(f"✓ Created {len(derived_features)} derived features:")
for feat in derived_features:
    print(f"  • {feat}")

# Use derived features as dynamic variables
...
# =========================================================================
# PHASE 3: STATIC VARIABLE SELECTION (on full dataset)
# =========================================================================
print("\n" + "="*100)
print("PHASE 3: STATIC VARIABLE SELECTION (FULL DATASET)")
print("="*100)
...
# -------------------------------------------------------------------------
# STEP 1: Spearman Decorrelation at Station Level (threshold = 0.90)
# -------------------------------------------------------------------------
def decorrelate_by_spearman_station_level(X_np, groups, col_names, threshold=0.90):
    """
...
print("\n" + "-"*100)
print("STEP 1: STATIC VARIABLES - Station-level decorrelation (ρ < 0.90)")
print("-"*100)

# Extract static features from FULL dataset
X_full_static_cols = np.array([c for c in static_present if c in X.columns])
X_full_static_np = X[X_full_static_cols.tolist()].to_numpy(dtype=np.float32)
groups_full = X['IDr'].to_numpy()

# Apply station-level decorrelation
static_decorrelated = decorrelate_by_spearman_station_level(
    X_full_static_np,
    groups_full,
    X_full_static_cols.tolist(),
    threshold=SPEARMAN_STATION_THRESHOLD
)

# -------------------------------------------------------------------------
# STEP 2: RFECV at Station Level (on FULL dataset)
# -------------------------------------------------------------------------
print("\n" + "-"*100)
print("STEP 2: RFECV at Station Level (FULL DATASET)")
print("-"*100)

if len(static_decorrelated) > 0:
    print(f"Preparing station-level data for RFECV...")
    
    # Aggregate static features by station (FULL dataset)
    Xstatic_obs = X[static_decorrelated].replace([np.inf, -np.inf], np.nan)
    ...
    print(f"  Y_station shape: {Y_station.shape}")
    
    # RFECV with ExtraTreesRegressor
    print(f"\nRunning RFECV with ExtraTreesRegressor...")
    ...
    static_drop = [c for c in static_decorrelated if c not in static_final]
    
    print(f"\n  RFECV Results:")
    print(f"    Input features: {len(static_decorrelated)}")
    print(f"    Selected features: {len(static_final)}")
    print(f"    Dropped features: {len(static_drop)}")
    print(f"    Best CV score: {rfecv.cv_results_['mean_test_score'].max():.6f}")
    ...
    rank_out = '../predict_score_red/RFECV_static_ranking_spearman_v1.txt'
    rank_df.to_csv(rank_out, sep=' ', index=False)
    print(f"    ✓ Saved ranking to: {rank_out}")
    ...
    curve_out = '../predict_score_red/RFECV_static_curve_spearman_v1.txt'
    curve_df.to_csv(curve_out, sep=' ', index=False)
    print(f"    ✓ Saved CV curve to: {curve_out}")
...
for v in dynamic_final[:15]:
    print(f"  • {v}")
if len(dynamic_final) > 15:
    print(f"  ... and {len(dynamic_final) - 15} more")

print(f"\nStatic variables: {len(static_final)}")
for v in sorted(static_final)[:15]:
    print(f"  • {v}")
if len(static_final) > 15:
    print(f"  ... and {len(static_final) - 15} more")
...
with open('../predict_importance_red/LSTM_selected_features_spearman_v1.txt', 'w') as f:
    ...
    f.write(f'VARIABLE_SELECTION: Full Dataset (before train/test split)\n')
    f.write(f'METHOD: Step 1 - Spearman Decorrelation (threshold={SPEARMAN_STATION_THRESHOLD})\n')
    f.write(f'METHOD: Step 2 - RFECV at Station Level\n')
    f.write('\nDYNAMIC_VARIABLES\n')
...
with open('../predict_score_red/LSTM_SPEARMAN_REPORT_v1.txt', 'w') as f:
    f.write('='*100 + '\n')
    f.write('LSTM WITH SPEARMAN DECORRELATION + RFECV PIPELINE REPORT\n')
    ...
    f.write('## FEATURE SELECTION (Full Dataset)\n')
    f.write('-'*60 + '\n')
    f.write(f'Step 1: Spearman station-level threshold: {SPEARMAN_STATION_THRESHOLD}\n')
    f.write(f'Step 2: RFECV at station level\n')
    f.write(f'Dynamic variables: {len(dynamic_final)}\n')
    f.write(f'Static variables (after decorrelation): {len(static_decorrelated)}\n')
    f.write(f'Static variables (after RFECV): {len(static_final)}\n')
