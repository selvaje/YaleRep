...
#SBATCH -n 1 -c 16  -N 1
#SBATCH -t 24:00:00
#SBATCH -o /vast/palmer/scratch/sbsc/ga254/stdout/sc31_RF_RFECV_MATRIX_acc2.sh.%A_%a.out
#SBATCH -e /vast/palmer/scratch/sbsc/ga254/stderr/sc31_RF_RFECV_MATRIX_acc2.sh.%A_%a.err
#SBATCH --job-name=sc31_RF_RFECV_MATRIX_acc2.sh
#SBATCH --array=500
#SBATCH --mem=500G
...
# targets: Q/area (no log)
q_cols = ['QMIN', 'Q10', 'Q20', 'Q30', 'Q40', 'Q50', 'Q60', 'Q70', 'Q80', 'Q90', 'QMAX']

Qtr = Y_train[q_cols].astype('float32')
Qte = Y_test[q_cols].astype('float32')

Yarea_train = (Qtr.div(acc_tr, axis=0)).astype('float32')
Yarea_test = (Qte.div(acc_te, axis=0)).astype('float32')

Y_train_np = Yarea_train.to_numpy()
Y_test_np = Yarea_test.to_numpy()

# RFECV station-level targets: mean(Q)/area + CV(std(Q)/mean(Q))
...
rank_out = f'../predict_score_red/RFECV_static_ranking_N{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_acc.txt'
...
curve_out = f'../predict_score_red/RFECV_static_curve_N{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_acc.txt'
curve_df.to_csv(curve_out, sep=' ', index=False)

# Train ET_SD on area-normalized targets
...
# Existing accuracy matrix in q space: YAscore*
train_A = compute_error_pack(Y_train_np, Y_train_pred)
test_A = compute_error_pack(Y_test_np, Y_test_pred)
...
np.savetxt(f'../predict_score_red/stationID_x_y_valueALL_predictors_YAscorerN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_r_A, delimiter=' ', fmt=fmt_score)
np.savetxt(f'../predict_score_red/stationID_x_y_valueALL_predictors_YAscorerhoN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_rho_A, delimiter=' ', fmt=fmt_score)
np.savetxt(f'../predict_score_red/stationID_x_y_valueALL_predictors_YAscoremaeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_mae_A, delimiter=' ', fmt=fmt_score)
np.savetxt(f'../predict_score_red/stationID_x_y_valueALL_predictors_YAscorekgeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_kge_A, delimiter=' ', fmt=fmt_score)

# NEW additional accuracy matrix in original Q space: QYAscore*
...
Q_train_pred = Y_train_pred.astype('float64') * acc_tr_np
Q_test_pred = Y_test_pred.astype('float64') * acc_te_np
...
np.savetxt(f'../predict_score_red/stationID_x_y_valueALL_predictors_QYAscorerN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_r_Q, delimiter=' ', fmt=fmt_score)
np.savetxt(f'../predict_score_red/stationID_x_y_valueALL_predictors_QYAscorerhoN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_rho_Q, delimiter=' ', fmt=fmt_score)
np.savetxt(f'../predict_score_red/stationID_x_y_valueALL_predictors_QYAscoremaeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_mae_Q, delimiter=' ', fmt=fmt_score)
np.savetxt(f'../predict_score_red/stationID_x_y_valueALL_predictors_QYAscorekgeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_kge_Q, delimiter=' ', fmt=fmt_score)
...
np.savetxt(
    ...
    Y_train_pred, delimiter=' ', fmt=fmt_pred, header=' '.join(q_cols), comments=''
)
np.savetxt(
    ...
    Y_test_pred, delimiter=' ', fmt=fmt_pred, header=' '.join(q_cols), comments=''
)
