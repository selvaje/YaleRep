...
### sbatch --export=var=snow,yyyy=1958  /gpfs/gibbs/pi/hydro/hydro/scripts/TERRA/sc09_for_run_singleVariable.sh
...
sbatch  --export=tif=$tif,ID=$ID --mem=${MEM}M --job-name=sc10_var_acc_intb1_TERRA_forloop_$(basename $tif .tif).sh  /gpfs/gibbs/pi/hydro/hydro/scripts/TERRA/sc10_variable_accumulation_intb1_TERRA_forloop.sh 
done 
done
sleep 1200
done
