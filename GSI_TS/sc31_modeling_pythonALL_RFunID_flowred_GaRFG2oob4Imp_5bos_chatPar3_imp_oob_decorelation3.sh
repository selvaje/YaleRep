...
class GroupAwareMultiOutput:
    def __init__(
        ...
        **kwargs
        ...
        self._groups = None  # ← persistent storage

    def fit(self, X, Y, groups=None, X_column_names=None):
        # Store groups if provided (only needs to be passed once)
        if groups is not None:
            self._groups = np.asarray(groups)

        # Require groups only if we actually do CV
        if self.n_cv_folds > 1 and self._groups is None:
            raise ValueError(
                'groups must be provided when n_cv_folds > 1 (required for group-aware cross-validation)'
            )
        ...
        if self.n_cv_folds > 1:
            ...
            # No CV — skip OOB calculation (used during RFECV internal fits)
            self.oob_predictions_ = None
            self.oob_r2_per_target_ = None
            self.oob_r2_mean_ = None
            self.oob_scores_ = None
    ...
    def print_oob_summary(self):
        if self.oob_r2_per_target_ is None:
            print('Model not fitted yet or no OOB computed (n_cv_folds=1).')
            return
...
selector_estimator = GroupAwareMultiOutput(
    ...
    n_cv_folds=1,  # No CV during feature selection to avoid nested CV overhead
    random_state=24,
    oob_metric='r2'
)
