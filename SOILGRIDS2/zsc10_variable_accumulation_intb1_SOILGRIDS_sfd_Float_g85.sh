...
#SBATCH -o /vast/palmer/scratch/sbsc/ga254/stdout/sc10_SOILGRIDS_sfd_Float_g85.sh.%J.out
#SBATCH -e /vast/palmer/scratch/sbsc/ga254/stderr/sc10_SOILGRIDS_sfd_Float_g85.sh.%J.err
#SBATCH --mem=400G
#SBATCH --job-name=sc10_SOILGRIDS_sfd_Float_g85.sh
...
/usr/local/grass-8.5/bin/grass -f --text --tmp-project $RAM/${tifname}_${box}_msk.tif  <<'EOF'
...
r.flowaccumulation input=dir type=DCELL weight=var_are   output=varare_acc nprocs=4  

export GDAL_CACHEMAX=100G
export GDAL_NUM_THREADS=4

r.out.gdal --o -f -c -m createopt='COMPRESS=DEFLATE,ZLEVEL=9,BIGTIFF=YES' nodata=-9999999 type=Float64   input=varare_acc  output=$SOILGRIDSH/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Float_g85.tif 

EOF
"
module load StdEnv 
source ~/bin/gdal3 &> /dev/null
gdalinfo -mm $SOILGRIDSH/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Float_g85.tif | grep Computed | awk '{ gsub(/[=,]/," " , $0 ); print $3 , $4 }' > $SOILGRIDSH/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Float_g85.mm
