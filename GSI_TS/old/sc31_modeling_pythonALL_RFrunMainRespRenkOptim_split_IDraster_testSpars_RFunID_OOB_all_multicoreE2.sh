...
Y = pd.read_csv(rf'stationID_x_y_valueALL_predictors_Ys11.txt', header=0, sep=' ', dtype=dtypes_Y, engine='c', low_memory=False)
X = pd.read_csv(rf'stationID_x_y_valueALL_predictors_Xs11.txt', header=0, sep=' ', usecols=lambda col: col not in excluded_columns, dtype=dtypes_X, engine='c',     low_memory=False )
...
del X, Y, stations, train_rasters, test_rasters #remove to save memory
gc.collect()
...
importance = pd.Series(RFreg.feature_importances_, index=X.columns[8:])
