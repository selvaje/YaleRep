...
# import slope 
GDAL_CACHEMAX 40000  
gdal_translate -a_srs EPSG:4326 -co BIGTIFF=YES  -co COMPRESS=DEFLATE -co ZLEVEL=9 -projwin $ulx $uly $lrx $lry /gpfs/scratch60/fas/sbsc/ga254/dataproces/MERIT/geomorphometry_90m_wgs84/slope/all_tif_slope.vrt  $RAM/${tile}_slope_$th_sp.tif
...
r.external  input=$RAM/${tile}_slope_$th_sp.tif     output=slope        --overwrite 
...
r.univar map=flow_mfd  percentile=10,20,30,40,50,60,70,80,90  -ge  output=$NH/flow_tiles_intb1_slope/flow_mfd_${zone}${tile}_sp${th_sp}.txt
r.univar map=flow_sfd  percentile=10,20,30,40,50,60,70,80,90  -ge  output=$NH/flow_tiles_intb1_slope/flow_sfd_${zone}${tile}_sp${th_sp}.txt

r.mapcalc "flow = if ( slope>$th_sp , flow_mfd , flow_sfd  ) "
# r.info map=slope >  $NH/flow_tiles_intb1_slope/slope$th_sp.txt
r.univar map=flow   percentile=10,20,30,40,50,60,70,80,90  -ge  output=$NH/flow_tiles_intb1_slope/flow_sfd_mfd_${zone}${tile}_sp${th_sp}.txt
rm -f $RAM/${tile}_slope_$th_sp.tif
...
r.out.gdal --o -c -m  createopt="COMPRESS=DEFLATE,ZLEVEL=9" type=UInt32 format=GTiff nodata=0 input=lbasin_clean  output=$NH/lbasin_tiles_intb1_slope/lbasin_${zone}${tile}_sp${th_sp}.tif
r.out.gdal --o -c -m  createopt="COMPRESS=DEFLATE,ZLEVEL=9" type=UInt32 format=GTiff nodata=0 input=stream        output=$NH/stream_tiles_intb1_slope/stream_${zone}${tile}_sp${th_sp}.tif

echo stream lbasin  | xargs -n 1 -P 2 bash -c $'
var=$1 
gdal_edit.py -a_ullr  $wS $nS $eS $sS  $NH/${var}_tiles_intb1_slope/${var}_${zone}${tile}_sp${th_sp}.tif
gdal_edit.py -tr 0.000833333333333333333333333333333333 -0.000833333333333333333333333333333333 $NH/${var}_tiles_intb1_slope/${var}_${zone}${tile}_sp${th_sp}.tif
' _
...
r.out.gdal --o -f -c -m createopt="COMPRESS=DEFLATE,ZLEVEL=9,BIGTIFF=YES,INTERLEAVE=BAND,TILED=YES"  nodata=-9999999  type=Float32 format=GTiff input=flow_mfd  output=$NH/flow_tiles_intb1_slope/flow_mfd_${zone}${tile}_sp${th_sp}.tif
r.out.gdal --o -f -c -m createopt="COMPRESS=DEFLATE,ZLEVEL=9,BIGTIFF=YES,INTERLEAVE=BAND,TILED=YES"  nodata=-9999999  type=Float32 format=GTiff input=flow_sfd  output=$NH/flow_tiles_intb1_slope/flow_sfd_${zone}${tile}_sp${th_sp}.tif
r.out.gdal --o -f -c -m createopt="COMPRESS=DEFLATE,ZLEVEL=9,BIGTIFF=YES,INTERLEAVE=BAND,TILED=YES"  nodata=-9999999  type=Float32 format=GTiff input=flow  output=$NH/flow_tiles_intb1_slope/flow_${zone}${tile}_sp${th_sp}.tif

gdal_edit.py -a_ullr  $wS $nS $eS $sS  $NH/flow_tiles_intb1_slope/flow_${zone}${tile}_sp${th_sp}.tif 
gdal_edit.py -tr 0.000833333333333333333333333333333333 -0.000833333333333333333333333333333333  $NH/flow_tiles_intb1_slope/flow_${zone}${tile}_sp${th_sp}.tif 
gdal_edit.py -a_ullr  $wS $nS $eS $sS  $NH/flow_tiles_intb1_slope/flow_${zone}${tile}_sp${th_sp}.tif
gdal_edit.py -tr 0.000833333333333333333333333333333333 -0.000833333333333333333333333333333333  $NH/flow_tiles_intb1_slope/flow_${zone}${tile}_sp${th_sp}.tif 
...
# pkfilter -nodata -9999999  -co COMPRESS=DEFLATE -co ZLEVEL=9  -ot Int32  -dx 10  -dy 10 -d 10 -f mean -i $NH/flow_tiles_intb1_slope/flow_${zone}${tile}_sp${th_sp}.tif  -o $NH/flow_tiles_intb1_slope/flow_${zone}${tile}_sp${th_sp}_10p.tif
# gdal_edit.py -a_nodata -9999999   $NH/flow_tiles_intb1_slope/flow_${zone}${tile}_sp${th_sp}_10p.tif
# pkgetmask -co  COMPRESS=DEFLATE -co ZLEVEL=9 -ot Byte -min -9999998 -max 99999999999999999999 -i $NH/flow_tiles_intb1_slope/flow_${zone}${tile}_sp${th_sp}.tif -o $NH/flow_tiles_intb1_slope/flow_${zone}${tile}_sp${th_sp}_msk.tif
