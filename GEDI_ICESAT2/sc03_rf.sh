...
#SBATCH -n 1 -c 1 -N 1
#SBATCH -t 3:00:00       # 
#SBATCH -o /gpfs/gibbs/pi/hydro/hydro/stdout1/sc03_rf.sh.%J.out
#SBATCH -e /gpfs/gibbs/pi/hydro/hydro/stderr1/sc03_rf.sh.%J.err
#SBATCH --job-name=sc03_rf.sh
#SBATCH --mem=80G
### -p scavenge

### sbatch --export=DIR=icesat2_66 /gpfs/gibbs/pi/hydro/hydro/scripts/GEDI_ICESAT2/sc03_rf.sh
...
# features = pd.read_csv('filter_tree_land_cover_30m.txt', header=None, sep=' ')
features = pd.read_csv('filter_tree_land_slope_over3m_30m.txt', header=None, sep=' ')
features.columns = ['gedi', 'ice_70', 'ice_75', 'ice_80', 'ice_85', 'ice_90', 'ice_95', 'tree_cover', 'land_cover', 'terrain_slope']
features.head(5)
feat = features.iloc[:,1:10].columns.values

# features[features.isna().any(axis=1)]  ## find the row contians NA
# aa[aa['gedi'].isna().any(axis=1)]
# aa.iloc[aa['gedi'].dropna()]
...
train_features, test_features, train_labels, test_labels = train_test_split(features, labels, test_size = 0.343, random_state = 60)
...
rf = RFReg(n_estimators=1000, max_features="log2", max_depth=100, max_samples=0.5, n_jobs=-1, random_state=42)
...
#### output train_df
train_df = pd.DataFrame({'X1':train_labels, 'X2':dic_pred['train']})
test_df  = pd.DataFrame({'X1':test_labels,  'X2':dic_pred['test']}) 
np.savetxt(r'train_df.txt', train_df, fmt='%.2f', delimiter='\t')
np.savetxt(r'test_df.txt',  test_df,  fmt='%.2f', delimiter='\t')

pp1 = PdfPages('scatter_tt4.pdf')
...
pp = PdfPages('importance_tt4.pdf')
plot3 = plt.figure()
...
awk '{ count = 0; for (i = 1; i <= NF; ++i) if ($i < 5) ++count; print count }'  point_inTile_s60w070.txt

awk 'NF == 8' temp.txt | awk '{if ($1>$2) print}'    ## print if the number of columns is not equal to 8 in each line
awk 'NF == 1'  filter_???????_tree_land.txt
