...
head -n $n $RAM/IDs_x_y_area_NOarea10_warea_$TILE.txt  > $RAM/IDs_x_y_area_NOarea10_warea_${TILE}_n$n.txt 
read IDs x y area <<< $( cat $RAM/IDs_x_y_area_NOarea10_warea_${TILE}_n$n.txt )

g.region n=$(echo  $y + 0.1 | bc  )  s=$(echo $y - 0.1 | bc )  e=$(echo $x + 0.1 | bc )  w=$(echo $x - 0.1 | bc )  

v.in.ascii in=$RAM/IDs_x_y_area_NOarea10_warea_${TILE}_n$n.txt out=x_y_orig$n  separator=space columns="Station integer, Lon double precision, Lat double precision, Area double precision" x=2 y=3 skip=0 --overwrite  
...
r.stream.snap accumulation=flow threshold=$(echo "$area  - ($area * 0.1 )" | bc) input=x_y_orig$n output=x_y_snap$n stream_rast=stream radius=30 --overwrite memory=10000   # snapping 
