...
#SBATCH -n 1 -c 1 -N 1
#SBATCH -t 2:00:00
#SBATCH -o /gpfs/scratch60/fas/sbsc/ga254/stdout/sc11_tiling20d_GRAND.sh.%A_%a.out  
#SBATCH -e /gpfs/scratch60/fas/sbsc/ga254/stderr/sc11_tiling20d_GRAND.sh.%A_%a.err
#SBATCH --array=1-116
#SBATCH --mem=22G
...
#### sbatch --export=year=1958,tifname=GRanD_1958 --job-name=sc11_tiling20d_GRAND_1958.sh  /gpfs/gibbs/pi/hydro/hydro/scripts/GRAND/sc11_tiling20d_GRAND.sh
...
#### gdalinfo  /gpfs/loomis/scratch60/sbsc/ga254/dataproces/GRAND/tmax_acc/2018/tmax_2018_04_intb.vrt
#### be sure that is always constant 

gdalbuildvrt -overwrite -srcnodata -9999999 -vrtnodata -9999999 $GRANDSC/${year}_acc/${tifname}_intb.vrt $(for ID in $(seq 1 59) ; do ls  $GRANDSC/${year}_acc/intb/${tifname}_*${ID}_acc.tif ; done) 

gdalbuildvrt -overwrite -srcnodata -9999999 -vrtnodata -9999999 $GRANDSC/${year}_acc/${tifname}_intb_5p.vrt $(for ID in $(seq 1 59) ; do ls $GRANDSC/${year}_acc/intb/${tifname}_*${ID}_acc_5p.tif ; done)
gdal_translate -a_nodata -9999999 -a_srs EPSG:4326 -co COMPRESS=DEFLATE -co ZLEVEL=9 -r mean -tr 0.008333333333333 0.008333333333333   $GRANDSC/${year}_acc/${tifname}_intb_5p.vrt $GRANDSC/${year}_acc/${tifname}_intb_5p.tif
rm -r $GRANDSC/${year}_acc/${tifname}_intb_5p.vrt
else 
sleep 300
fi

gdal_translate -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -co TILED=YES -co NUM_THREADS=2 -a_nodata -9999999 -ot Float32 -projwin $ulx $uly $lrx $lry $GRANDSC/${year}_acc/${tifname}_intb.vrt $RAM/${tifname}_${tile}_acc.tif
...
pksetmask  -m $RAM/${tifname}_${tile}_acc.tif -msknodata -9999999 -nodata -9999999 -m  $RAM/flow_${tifname}_${tile}.tif  -msknodata -9999999 -nodata -9999999 -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -co TILED=YES -co NUM_THREADS=2 -ot Int32 -i $RAM/${tifname}_${tile}_acc_tmp.tif -o $GRANDH/${year}_acc/tiles20d/${tifname}_${tile}_acc.tif

echo ${tifname}_${tile}_acc.tif  $( pkstat -hist   -src_min -9999999.1 -src_max -9999998.9 -i $GRANDH/${year}_acc/tiles20d/${tifname}_${tile}_acc.tif  | awk '{ print $2 }' ) > /dev/shm/${tifname}_${tile}_acc.nd 

#### in case of no data put 0 ; the tiles is cover by full data value
awk '{ if($2=="") { print $1 , 0 } else {print $1 , $2 } }' /dev/shm/${tifname}_${tile}_acc.nd   > $GRANDH/${year}_acc/tiles20d/${tifname}_${tile}_acc.nd
...
## rm -f  $GRANDSC/${year}_acc/intb/${tifname}_???_acc.tif 
#### 
sleep 1000  # to allow that the sc10 form other month are keep running.


yearless=$( expr ${year}_acc - 1 )
echo   $GRANDSC/${yearless}_acc/intb/${dir}_Y${yearless}_*_acc.tif  
rm  -f $GRANDSC/${yearless}_acc/intb/${dir}_Y${yearless}_*_acc.tif  ### rm accumualtion grass tiles of the previus 1 month  year

# yearplus=$( expr $year + 1 )
# for tif in /gpfs/gibbs/pi/hydro/hydro/dataproces/GRAND/${dir}/${dir}_Y${yearplus}.tif ; do for ID in $(awk '{ print $1 }' /gpfs/gibbs/pi/hydro/hydro/dataproces/MERIT_HYDRO/tiles_comp/tileComp_size_memory.txt  )  ; do MEM=$(grep ^"$ID " /gpfs/gibbs/pi/hydro/hydro/dataproces/MERIT_HYDRO/tiles_comp/tileComp_size_memory.txt | awk  '{ print $4}' ) ;  sbatch  --export=tif=$tif,ID=$ID --mem=${MEM}M --job-name=sc10_var_acc_intb1_GRAND_forloop_$(basename $tif .tif).sh  /gpfs/gibbs/pi/hydro/hydro/scripts/GRAND/sc10_variable_accumulation_intb1_GRAND_forloop.sh ; sleep 2 ; done ; done  

gdalbuildvrt -overwrite -srcnodata -9999999 -vrtnodata -9999999 $GRANDH/${year}_acc/${tifname}.vrt $GRANDH/${year}_acc/tiles20d/${tifname}_*_acc.tif
gdal_translate -a_nodata -9999999 -a_srs EPSG:4326 -co COMPRESS=DEFLATE -co ZLEVEL=9 -r bilinear -tr 0.0083333333333 0.0083333333333 $GRANDH/${year}_acc/${tifname}.vrt $GRANDH/${year}_acc/${tifname}.tif 
