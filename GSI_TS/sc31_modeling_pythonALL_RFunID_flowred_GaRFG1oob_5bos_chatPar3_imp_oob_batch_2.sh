...
#### for obs_leaf in 100  ; do for obs_split in 100 ; do for sample in 0.9  ; do sbatch --export=obs_leaf=$obs_leaf,obs_split=$obs_split,sample=$sample  /gpfs/gibbs/pi/hydro/hydro/scripts/GSI_TS/sc31_modeling_pythonALL_RFunID_flowred_GaRFG1oob_5bos_chatPar3_imp_oob_batch.sh   ; done; done ; done 
...
class GroupAwareDecisionTree(DecisionTreeRegressor):
    def fit(self, X, y, groups=None, sample_weight=None, check_input=True):
        self.groups_ = groups  # store groups for debugging if needed
        super().fit(X, y, sample_weight=sample_weight, check_input=check_input)
        return self

    def predict(self, X):
        y_pred = super().predict(X)
        return np.clip(y_pred, 0, None)  # ensure non-negative
...
class GroupAwareRandomForest:
    ...
    def _fit_single_tree(self, tree, X, y, groups, rng):
        n_samples = X.shape[0]

        if groups is not None:
            unique_groups = np.unique(groups)
            sampled_groups = rng.choice(unique_groups, size=len(unique_groups), replace=True)
            in_bag_idx = np.hstack([np.where(groups == g)[0] for g in sampled_groups])
            in_bag_idx = np.unique(in_bag_idx)
            oob_idx = np.setdiff1d(np.arange(n_samples), in_bag_idx)
        else:
            in_bag_idx = rng.choice(n_samples, n_samples, replace=True)
            oob_mask = np.ones(n_samples, dtype=bool)
            oob_mask[in_bag_idx] = False
            oob_idx = np.where(oob_mask)[0]

        X_boot, y_boot = X[in_bag_idx], y[in_bag_idx]
        groups_boot = groups[in_bag_idx] if groups is not None else None

        tree.fit(X_boot, y_boot, groups=groups_boot)
    ...
    def fit(self, X, y, groups=None):
        ...
        n_samples, n_outputs = y.shape if y.ndim > 1 else (y.shape[0], 1)

        # Create decision trees with max_features passed
        trees = [
            GroupAwareDecisionTree(
        ...
        results = Parallel(n_jobs=self.n_jobs)(
            delayed(self._fit_single_tree)(tree, X, y, groups, rng)
            for tree in trees
        )
        self.estimators_, oob_indices_all = zip(*results)

        # OOB score
        if self.oob_metric is not None:
            oob_preds = np.zeros((n_samples, n_outputs))
            oob_counts = np.zeros(n_samples)

            for tree, oob_idx in zip(self.estimators_, oob_indices_all):
                if len(oob_idx) == 0:
                    continue
                preds = tree.predict(X[oob_idx])
                preds = preds.reshape(-1, n_outputs)
                oob_preds[oob_idx] += preds
                oob_counts[oob_idx] += 1

            mask = oob_counts > 0
            oob_final = np.zeros_like(oob_preds)
            oob_final[mask] = oob_preds[mask] / oob_counts[mask, None]

            if self.oob_metric == 'r2':
                if n_outputs == 1:
                    self.oob_scores_ = r2_score(y[mask], oob_final[mask])
                else:
                    per_output = [
                        r2_score(y[mask, j], oob_final[mask, j])
                        for j in range(n_outputs)
                    ]
                    global_r2 = r2_score(
                        y[mask], oob_final[mask], multioutput='variance_weighted'
                    )
                    self.oob_scores_ = [global_r2] + per_output
