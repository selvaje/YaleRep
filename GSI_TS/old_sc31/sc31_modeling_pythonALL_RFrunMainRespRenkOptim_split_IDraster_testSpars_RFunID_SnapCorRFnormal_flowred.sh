...
#### for obs_leaf 50  ; do for obs_split in 50  ; do for sample in 0.9  ; do sbatch --export=obs_leaf=$obs_leaf,obs_split=$obs_split,sample=$sample /gpfs/gibbs/pi/hydro/hydro/scripts/GSI_TS/sc31_modeling_pythonALL_RFrunMainRespRenkOptim_split_IDraster_testSpars_RFunID_SnapCorRFnormal_flowred.sh  ; done; done ; done 
...
# Verify that X and Y have matching IDr values
assert (X['IDr'] == Y['IDr']).all(), 'IDr values in X and Y do not match!'

# Perform IDr-based train-test split
train_rasters, test_rasters = train_test_split(stations['IDr'], test_size=0.2, random_state=24)

# Get indices for train and test sets based on IDr
train_indices = X[X['IDr'].isin(train_rasters)].index
test_indices = X[X['IDr'].isin(test_rasters)].index
...
# Debug: Check first few rows for alignment
print('First few IDr values in X_train:', X_train['IDr'].head().to_list())
print('First few IDr values in Y_train:', Y_train['IDr'].head().to_list())
print('First few IDr values in X_test:', X_test['IDr'].head().to_list())
print('First few IDr values in Y_test:', Y_test['IDr'].head().to_list())
...
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YRFscorerN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_2RF.txt', merge_r, delimiter=' ', fmt=fmt)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YRFscorerhoN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_2RF.txt', merge_rho, delimiter=' ', fmt=fmt)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YRFscoremaeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_2RF.txt', merge_mae, delimiter=' ', fmt=fmt)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YRFscorekgeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_2RF.txt', merge_kge, delimiter=' ', fmt=fmt)
