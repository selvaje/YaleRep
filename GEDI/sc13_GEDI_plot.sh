...
#SBATCH -n 1 -c 1 -N 1
#SBATCH -t 1:00:00       # 
#SBATCH -o /gpfs/gibbs/pi/hydro/hydro/stdout1/sc13_GEDI_plot.sh.%J.out
#SBATCH -e /gpfs/gibbs/pi/hydro/hydro/stderr1/sc13_GEDI_plot.sh.%J.err
#SBATCH --job-name=sc13_GEDI_plot.sh
#SBATCH --mem=2G
##x_y_sensitivity 

### for string in x_y_day x_y_coveragebeam x_y_degrade ; do sbatch --export=string=$string /gpfs/gibbs/pi/hydro/hydro/scripts/GEDI/sc13_GEDI_plot.sh ; done 
...
if [ $string = x_y_allfilter ]       ;  then export DIR=af  ; export labely="All Filtered" ;  fi 
if [ $string = x_y_day ]             ;  then export DIR=dy  ; export labely="During day" ; fi 
if [ $string = x_y_coveragebeam ]    ;  then export DIR=cb  ; export labely="Coverage beams" ; fi 
if [ $string = x_y_sensitivity ]     ;  then export DIR=st  ; export labely="Sensitivity" ; fi 
if [ $string = x_y_degrade ]         ;  then export DIR=de  ; export labely="Degrade" ; fi 
