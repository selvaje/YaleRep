...
try:
    with h5py.File(FILE_NAME, mode='r') as f:
        ...
        for g in group:
            ...
            segment_landcover = f[g+'/land_segments/segment_landcover'][:]

            # get the rh_25, rh_50, rh_60, rh_70, rh_75, rh_80, rh_85, rh_90, rh_95
            col_name = ['rh_10', 'rh_15', 'rh_20', 'rh_25', 'rh_30', 'rh_35', 'rh_40', 'rh_45', 'rh_50', 'rh_55', 'rh_60', 'rh_65', 'rh_70', 'rh_75', 'rh_80', 'rh_85', 'rh_90', 'rh_95']
            canopy_perc_df = pd.DataFrame(np.array(canopy_perc).reshape(len(canopy_perc),18), columns = col_name)
            
            col_name = ['rh_25', 'rh_50', 'rh_60', 'rh_70', 'rh_75', 'rh_80', 'rh_85', 'rh_90', 'rh_95']
            canopy_perc_df = canopy_perc_df[col_name]
            ...
            geoDF = pd.DataFrame({'latitude': latitude, 'longitude': longitude, 'rh_98': canopy98, 'min_canopy': min_canopy, 'night_f': night_f, 'sd_canopy': sd_canopy, 'segment_landcover': segment_landcover})
            ...
            icesatDF = pd.concat([icesatDF, geoDF])
            del geoDF
        ...
        icesatDF = icesatDF.drop(columns=['geometry', 'sd_canopy'])

        # check if icesatDF is empty
        if len(icesatDF) == 0:
            print(f"{FILE_NAME} No points were founded")
            x=np.array(['nopoint'])
            np.savetxt(f"{outDir}{FILE_NAME.replace('.h5', '_nopoint.txt')}", x , fmt="%s" )
            sys.exit(3)
        else:
          ...
          # Check for empty output dataframe
            try:    
                # fmt = (len(icesatDF.columns)-6) * ['%.2f']
                fmt = (len(icesatDF.columns)-6) * ['%.2f']
                fmt[0:0] = ['%.8g', '%.8g', '%.2f', '%.2f', '%d', '%d']
    ...
    x=np.array(['h5error'])
    np.savetxt(f"{outDir}{FILE_NAME.replace('.h5', '_h5error.txt')}", x , fmt="%s" )
    sys.exit(4)
