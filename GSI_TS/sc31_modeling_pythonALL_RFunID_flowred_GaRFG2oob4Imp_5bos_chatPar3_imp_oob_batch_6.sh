...
class GroupAwareDecisionTree(DecisionTreeRegressor):
    ...
    def fit(self, X, y, groups=None, sample_weight=None, check_input=True):
        self.groups_ = groups  # stored but not used in splitting
        return super().fit(X, y, sample_weight=sample_weight, check_input=check_input)

    def predict(self, X):
        y_pred = super().predict(X)
        return np.clip(y_pred, 0, None)  # enforce non-negative outputs


# -------------------------------
# Group-Aware Random Forest v2
# -------------------------------
class GroupAwareRandomForest2:
    def __init__(self,
                 ...
                 min_samples_leaf=1,
                 max_features='sqrt',
                 bootstrap_frac=0.7,     # fraction of rows sampled per group
                 n_jobs=-1,
                 random_state=None,
                 oob_metric=None,
                 gc_interval=10):
    ...
    def _fit_single_tree(self, X, y, groups, rng, tree_idx):
        ...
        if groups is not None:
            ...
            for g in sampled_groups:
                ...
                # Sample only a fraction of the group's rows
        ...
        # OOB = everything not in bootstrap
        oob_idx = np.setdiff1d(np.arange(n_samples), train_idx)

        X_boot, y_boot = X[train_idx], y[train_idx]

        # Independent RNG per tree
        tree_rng = np.random.default_rng(rng.integers(0, 1e9))
        tree = GroupAwareDecisionTree(
            max_depth=self.max_depth,
            min_samples_split=self.min_samples_split,
            min_samples_leaf=self.min_samples_leaf,
            max_features=self.max_features,
            random_state=tree_rng.integers(0, 1e9)
        )
    ...
    def fit(self, X, y, groups=None):
        ...
        if self.oob_metric is not None:
            ...
            mask = np.any(preds_count > 0, axis=1)
            if np.any(mask):
                ...
                if self.oob_metric == 'r2':
                    ...
                    per_output_r2 = [
                        r2_score(y[mask, j], oob_preds[:, j]) for j in range(y.shape[1])
                    ]
