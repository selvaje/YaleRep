...
class GroupAwareRandomForest2:
    ...
    def _fit_single_tree(self, X, y, groups, rng, tree_idx):
        n_samples = X.shape[0]
        draw_n = max(1, int(n_samples * self.bootstrap_frac))
        train_idx = rng.choice(n_samples, size=draw_n, replace=True)
        ...
        # Identify groups that tree saw in its bootstrap
        if groups is not None:
            seen_groups = np.unique(groups[train_idx])
        else:
            seen_groups = None
...
RFreg = GroupAwareRandomForest2(
    ...
    max_features='sqrt',
    max_depth=20,                   # or lower (10–12) to reduce overfit
