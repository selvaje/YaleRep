...
# =========================================================================
# PHASE 7: TEMPORAL SPLIT (IMPROVED - FIXED)
# =========================================================================
print("\n" + "="*100)
print("PHASE 7: TEMPORAL SPLIT")
print("="*100)
...
print(f"Time range: {min_year}-{max_year} ({max_year - min_year + 1} years)")
...
Y_train_temp = Y[train_mask].copy()
X_test_temp = X[test_mask].copy()
Y_test_temp = Y[test_mask].copy()

print(f"\nInitial split:")
print(f"  Train: X {X_train_temp.shape}, Y {Y_train_temp.shape}, Stations: {X_train_temp['station_id'].nunique()}")
print(f"  Test:  X {X_test_temp.shape}, Y {Y_test_temp.shape}, Stations: {X_test_temp['station_id'].nunique()}")

# Check consecutive months in EACH split separately
print(f"\n{'='*100}")
print(f"FILTERING STATIONS (MIN {SEQ_LEN} CONSECUTIVE MONTHS IN BOTH SPLITS)")
print(f"{'='*100}")

print("\nChecking train split...")
train_station_months = check_consecutive_months(X_train_temp, SEQ_LEN)
valid_train_stations = {sid for sid, months in train_station_months.items() if months >= SEQ_LEN}
print(f"  Stations with ≥{SEQ_LEN} consecutive months in TRAIN: {len(valid_train_stations)}")

print("\nChecking test split...")
test_station_months = check_consecutive_months(X_test_temp, SEQ_LEN)
valid_test_stations = {sid for sid, months in test_station_months.items() if months >= SEQ_LEN}
print(f"  Stations with ≥{SEQ_LEN} consecutive months in TEST: {len(valid_test_stations)}")

# Keep only stations valid in BOTH splits
valid_both = list(valid_train_stations & valid_test_stations)

print(f"\nStation filtering results:")
print(f"  Total before filtering: {X['station_id'].nunique()}")
print(f"  Valid in train only: {len(valid_train_stations - valid_test_stations)}")
print(f"  Valid in test only: {len(valid_test_stations - valid_train_stations)}")
print(f"  Valid in BOTH (kept): {len(valid_both)}")

if len(valid_both) == 0:
    print(f"\n❌ ERROR: No stations have {SEQ_LEN} consecutive months in BOTH train and test!")
    print(f"\nDiagnostics:")
    print(f"  Train period: {TRAIN_START}-{TRAIN_END} ({TRAIN_YEARS} years)")
    print(f"  Test period: {TEST_START}-{TEST_END} ({TEST_YEARS} years)")
    print(f"  Required: {SEQ_LEN} months ({SEQ_LEN/12:.1f} years) consecutive")
    sys.exit(1)

# Filter to valid stations
print(f"\nFiltering to {len(valid_both)} valid stations...")
X = X[X['station_id'].isin(valid_both)].reset_index(drop=True)
Y = Y[Y['station_id'].isin(valid_both)].reset_index(drop=True)

# Redo split with filtered data
train_mask = (X['YYYY'] >= TRAIN_START) & (X['YYYY'] <= TRAIN_END)
test_mask = (X['YYYY'] >= TEST_START) & (X['YYYY'] <= TEST_END)
...
print(f"\nFinal split:")
print(f"  Train: X {X_train.shape}, Y {Y_train.shape}, Stations: {X_train['station_id'].nunique()}")
print(f"  Test:  X {X_test.shape}, Y {Y_test.shape}, Stations: {X_test['station_id'].nunique()}")

del X_train_temp, Y_train_temp, X_test_temp, Y_test_temp
del train_mask, test_mask, train_station_months, test_station_months
gc.collect()
