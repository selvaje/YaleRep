...
class BoundedGroupAwareRandomForest(RandomForestRegressor, RegressorMixin):
    def fit(self, X, Y):
        self.oob_predictions = np.full(Y.shape, fill_value=np.nan, dtype=np.float64)  # OOB storage
...
### Compute OOB error for each Q (QMIN to QMAX) at the group level.
    def compute_oob_error(self, Y_true):
        ...
        oob_errors = np.array([mean_squared_error(Y_true.loc[valid_mask, col], self.oob_predictions[valid_mask, i])
                               for i, col in enumerate(Y_true.drop(columns=['IDraster']).columns)])
        overall_oob_error = np.mean(oob_errors)
...
fmt = '%i %2.f %2.f %2.f %2.f %2.f %2.f %2.f %2.f %2.f %2.f %2.f'
...
savetxt(rf'./stationID_x_y_valueALL_predictors_OOBrID_N{N_EST_S}_{obs_s}obs_IDr.txt', oob_errors , delimiter=' ', fmt='%2.f', comments='')
savetxt(rf'./stationID_x_y_valueALL_predictors_OOBrID_N{N_EST_S}_{obs_s}obs_all.txt', [overall_oob_error] , delimiter=' ', fmt='%2.f', comments='')
