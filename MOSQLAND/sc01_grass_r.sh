...
awk -F "," '{ if (NR>1)  print NR-1 , $1   }'   $OUT_TXT/FST_list_NAmRF3_${VAR}$point.csv | xargs -n 2 -P 16  bash -c $' 
CAT=$1
INDEX=$2 
...
Shrubs + Herb + Crop + Flood + Urban + Snow + Barren + Water + Slope + Altitude + PET + DailyTempRange + max.temp + AnnualTempRange + prec.wet + prec.dry + GPP + kernel100, 
importance=TRUE, mtry = mtry_opt, na.action=na.omit, data=Env.table.test)
...
cat $OUT_TXT/FST_line_NAmRF3_StartStop${VAR}$point.txt   | xargs -n 5 -P 16  bash -c $'
INDEX=$1 
