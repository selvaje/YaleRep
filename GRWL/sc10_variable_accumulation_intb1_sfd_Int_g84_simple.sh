...
#SBATCH -o /vast/palmer/scratch/sbsc/ga254/stdout/sc10_GRWL_sfd_Int_g84_s.sh.%J.out
#SBATCH -e /vast/palmer/scratch/sbsc/ga254/stderr/sc10_GRWL_sfd_Int_g84_s.sh.%J.err

#### AF AU EUA GL NA NAO SA SAO SIO SPO

#### for tif in /gpfs/gibbs/pi/hydro/hydro/dataproces/GRWL/GRWL_{water,lake,delta,canal,river}.tif ; do for BOX in AF AU EUA GL NA NAO SA SAO SIO SPO ; do MEM=$(grep ^"$BOX " /gpfs/gibbs/pi/hydro/hydro/dataproces/SOILGRIDS2/mem_request_BOX.txt | awk '{ print $2}'); sbatch  --export=tif=$tif,BOX=$BOX --mem=${MEM} --job-name=sc10_sfd_$(basename $tif _WeAv_transGrow.vrt)_$BOX.sh /gpfs/gibbs/pi/hydro/hydro/scripts/GRWL/sc10_variable_accumulation_intb1_sfd_Int_g84_simple.sh ; done; done
...
export  dir=$(echo $tifname | cut -d "_"  -f 2 )
...
if [ ! -e $GRWLSC/$dir/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Int_g84.tif ] || [ ! -s $GRWLSC/$dir/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Int_g84.mm ]; then
...
# cp $RAM/${tifname}_${box}_var.tif $GRWLSC/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_var.tif 
# cp $GRWLH/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_var.tif  $RAM/${tifname}_${box}_var.tif 
...
GDAL_CACHEMAX=$MEMG
GDAL_NUM_THREADS=2
gdal_translate -of VRT -projwin $xmin $ymax  $xmax $ymin $GRWLSH/GRWL_mask_V01.01_wgs84_tif/all_tif_dis.vrt $RAM/${tifname}_${box}${tile}_tmp.vrt 
...
pkreclass -ot Byte -co COMPRESS=DEFLATE -co ZLEVEL=9 -co NUM_THREADS=2 -of GTiff -c 255 -r 1 -c 180 -r 0 -c 126 -r 0 -c 86 -r 0 -i $RAM/${tifname}_${box}_tmp.vrt -o $RAM/${tifname}_${box}_var.tif ; fi 
if [ $tifname = GRWL_lake ] ; then 
pkreclass -ot Byte -co COMPRESS=DEFLATE -co ZLEVEL=9 -co NUM_THREADS=2 -of GTiff -c 255 -r 0 -c 180 -r 1 -c 126 -r 0 -c 86 -r 0 -i $RAM/${tifname}_${box}_tmp.vrt -o $RAM/${tifname}_${box}_var.tif ; fi 
if [ $tifname = GRWL_delta ] ; then 
pkreclass -ot Byte -co COMPRESS=DEFLATE -co ZLEVEL=9 -co NUM_THREADS=2 -of GTiff -c 255 -r 0 -c 180 -r 0 -c 126 -r 1 -c 86 -r 0 -i $RAM/${tifname}_${box}_tmp.vrt -o $RAM/${tifname}_${box}_var.tif ; fi 
if [ $tifname = GRWL_canal ] ; then 
pkreclass -ot Byte -co COMPRESS=DEFLATE -co ZLEVEL=9 -co NUM_THREADS=2 -of GTiff -c 255 -r 0 -c 180 -r 0 -c 126 -r 0 -c 86 -r 1 -i $RAM/${tifname}_${box}_tmp.vrt -o $RAM/${tifname}_${box}_var.tif ; fi 
if [ $tifname = GRWL_water ] ; then 
pkreclass -ot Byte  -co COMPRESS=DEFLATE -co ZLEVEL=9 -co NUM_THREADS=2 -of GTiff -c 255 -r 1 -c 180 -r 1 -c 126 -r 1 -c 86 -r 1 -i $RAM/${tifname}_${box}_tmp.vrt -o $RAM/${tifname}_${box}_var.tif ; fi 
rm -f $RAM/${tifname}_${box}_tmp.vrt

# resampling the grwl by sum so max pixel value 9 

gdalwarp  -s_srs EPSG:4326 -t_srs EPSG:4326  -r sum -ot Byte  -tr 0.000833333333333333333 0.000833333333333333333 -co BIGTIFF=YES -co COMPRESS=ZSTD  -co ZSTD_LEVEL=9  -co INTERLEAVE=BAND -co NUM_THREADS=8 -co TILED=YES -multi -wo NUM_THREADS=8 $RAM/${tifname}_${box}_var.tif $RAM/${tifname}_${box}_var_sum.tif
gdalinfo -mm $RAM/${tifname}_${box}_var.tif
gdalinfo -mm $RAM/${tifname}_${box}_var_sum.tif
gdal_edit.py -a_nodata 255  $RAM/${tifname}_${box}_var_sum.tif 
...
/home/ga254/.grass8/addons/scripts/r.mapcalc.tiled   'var_are = float(are / 9.0 * var * 10000.0 )'   nprocs=8 # out of the water shoulb 0 
echo var_are
r.info -r var_are
...
gdalinfo -mm /tmp/${tifname}_${box}_acc_sfd_Int_g84.tif | grep Computed | awk '{ gsub(/[=,]/," " , $0 ); print $3 , $4 }' > $GRWLSC/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Int_g84.mm
gdal_translate -a_nodata 0  -co COMPRESS=ZSTD  -co ZSTD_LEVEL=9 -co BIGTIFF=YES -co NUM_THREADS=4 -co TILED=YES /tmp/${tifname}_${box}_acc_sfd_Int_g84.tif $GRWLSC/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Int_g84.tif 
ls -lh  /tmp/${tifname}_${box}_acc_sfd_Int_g84.tif 
ls -lh $GRWLSC/${dir}/${dir}_acc_sfd/intb/${tifname}_${box}_acc_sfd_Int_g84.tif 
...
for DIR in  ; do
    ...
    GRWL_{water,lake,delta,canal,river}.tif

    for BOX in EUA AF AU GL NA NAO SA SAO SIO SPO ; do
ll /vast/palmer/scratch/sbsc/hydro/dataproces/GRWL/$DIR/*_acc_sfd/intb/*_${BOX}_acc_sfd_Int_g84.tif
done
echo " " 
done 

#### memory luckup table
for n in $(grep _CELL /vast/palmer/scratch/sbsc/ga254/stderr/sc10_GRWL_sfd_Int_g84_s.sh.*.err | tr "." " " | cut -d " " -f 3 | sort | uniq ) ; do
    echo $(head -1 /vast/palmer/scratch/sbsc/ga254/stderr/sc10_GRWL_sfd_Int_g84_s.sh.$n.err | cut -d "/" -f 10 | cut -d "_" -f 1) $(seff $n | grep "Average Memory Usag" | awk '{ print int($4 + 50)"G"}')
done > /gpfs/gibbs/pi/hydro/hydro/dataproces/GRWL/mem_request_BOX.txt 
