...
class ClusteredRandomForestRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, 
                 ...
                 min_samples_split=2, 
                 random_state=None, 
                 n_jobs=-1,       # parallel jobs (only for prediction now)
    ...
    def fit(self, X, y):
        ...
        self.estimators_ = []
        for s in seeds:
            est = self._fit_single_tree(X_noID, y, self.groups_, s)
            self.estimators_.append(est)
            gc.collect()  # cleanup after each estimator to save memory
    ...
    def predict(self, X):
        ...
        preds = Parallel(n_jobs=self.n_jobs, backend='loky', verbose=0)(
        ...
        return np.maximum(np.mean(preds_stack, axis=0), 0)  # clip at zero
