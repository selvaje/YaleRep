...
#### for obs in 20 30 40 50 ; do for samp in 0 1 2 3 4 ;   do sbatch --export=obs=$obs,samp=$samp /gpfs/gibbs/pi/hydro/hydro/scripts/GSI_TS/sc30_modeling_pythonALL_RFrunMainRespRenk_RFunID_OOB_all_multicoreE_sampleImportance_flowred.sh ; done ; done 
#### 2 4 5 8 10 15 
...
X_train  = pd.read_csv(rf'stationID_x_y_valueALL_predictors_sampM{samp_s}_Xs.txt', header=0, sep=' ', usecols=lambda column: column not in ['IDs','YYYY','MM','Xsnap','Ysnap','Xcoord','Ycoord'], dtype=dtypes_X)
Y_train  = pd.read_csv(rf'./stationID_x_y_valueALL_predictors_sampM{samp_s}_Ys.txt', header=0, sep=' ', usecols=lambda column: column not in ['IDs','YYYY','MM','Xsnap','Ysnap','Xcoord','Ycoord'], dtype=dtypes_Y)
