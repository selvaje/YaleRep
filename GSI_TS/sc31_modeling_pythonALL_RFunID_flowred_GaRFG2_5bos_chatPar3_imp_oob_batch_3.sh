...
class GroupAwareDecisionTree(DecisionTreeRegressor):
    def fit(self, X, y, groups=None, sample_weight=None, check_input=True):
        # groups stored for reference only
        self.groups_ = groups  
    ...
    def predict(self, X):
        ...
        # Clip negatives (hydrology discharge cannot be < 0)
        y_pred[y_pred < 0] = 0
        return y_pred
...
class GroupAwareRandomForest:
    ...
    def _fit_single_tree(self, X, y, groups, rng):
        ...
        if groups is not None:
            ...
            # Collect all indices belonging to those groups
            indices = np.hstack([np.where(groups == g)[0] for g in sampled_groups])
        else:
            # Standard bootstrap if groups not provided
            n_samples = X.shape[0]
    ...
    def predict(self, X):
        ...
        # Hydrology sanity check: no negative discharge
        y_pred[y_pred < 0] = 0
        return y_pred
