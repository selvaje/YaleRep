...
if [ $ID -eq  41  ] ; then     ### change back to 48 
sbatch  --export=dir=$dir,year=$year,tifname=$tifname  --job-name=sc11_tiling20d_TERRA_${tifname}_tile.sh    --dependency=afterany:$(squeue -u $USER -o "%.9F %.80j" | grep  sc10_var_acc_intb1_TERRA_forloop_${tifname}.sh | awk '{ printf ("%i:" , $1)} END {gsub(":","") ; print $1 }'  ) /gpfs/gibbs/pi/hydro/hydro/scripts/TERRA/sc11_tiling20d_TERRA_tile.sh
sleep 30
fi
