...
def compute_descriptive_stats(df, name, target_cols=None):
    ...
    show_cols = target_cols if target_cols else numeric_cols[:20]
    show_cols = [c for c in show_cols if c in df.columns]

    if show_cols:
        desc = df[show_cols].describe(percentiles=[0.01, 0.05, 0.25, 0.50, 0.75, 0.95, 0.99])
        print(f'\nDescriptive statistics ({name}):')
        print(desc.to_string())

    if 'YYYY' in df.columns:
        years = df['YYYY'].unique()
        print(f'\nTemporal range: {years.min()} - {years.max()} ({len(years)} unique years)')
    if 'MM' in df.columns:
        months = sorted(df['MM'].unique().tolist())
        print(f'Months present: {months}')

    if target_cols:
        print(f'\nOutlier detection (3xIQR rule) for target columns:')
        for col in target_cols:
            if col in df.columns:
                q1 = df[col].quantile(0.25)
                q3 = df[col].quantile(0.75)
                iqr = q3 - q1
                outliers = int(((df[col] < q1 - 3 * iqr) | (df[col] > q3 + 3 * iqr)).sum())
                print(f'  {col}: {outliers} potential outliers')

    if 'IDr' in df.columns:
        n_groups = df['IDr'].nunique()
        obs_per_group = df['IDr'].value_counts()
        print(f'\nGroup (IDr) summary: {n_groups} unique groups')
        print(f'  Obs per group - min: {obs_per_group.min()}, median: {obs_per_group.median():.0f}, max: {obs_per_group.max()}')

x_dynamic_cols = [c for c in dynamic_var if c in X.columns]
x_static_cols = [c for c in static_var if c in X.columns]
y_target_cols = ['QMIN', 'Q10', 'Q20', 'Q30', 'Q40', 'Q50', 'Q60', 'Q70', 'Q80', 'Q90', 'QMAX']

compute_descriptive_stats(X, 'X sample', target_cols=x_dynamic_cols[:10])
compute_descriptive_stats(Y, 'Y sample', target_cols=y_target_cols)
...
# Bug fix: correct format specifiers matching Y column dtypes
# Y columns: IDs(int) IDr(int) YYYY(int) MM(int) Xsnap(float) Ysnap(float) Xcoord(float) Ycoord(float) + 11 quantile floats
fmt_Y = '%i %i %i %i %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f'
...
class GroupAwareMultiOutput(BaseEstimator, RegressorMixin):
    '''
    Group-aware multi-output regressor with memory-efficient OOB cross-validation.

    Explicitly handles temporal variation within IDr groups via dynamic feature
    variance analysis. Uses memory-efficient group folding for 11M+ rows.
    Computes per-group temporal statistics to inform fold splits and provides
    robust OOB estimation without data leakage.
    '''
    ...
    def __init__(self, base_estimator, n_cv_folds=5, n_jobs=1, inner_n_jobs=1,
        ...
        self.group_temporal_stats_ = None
        self._groups = None
        self.X_column_names_ = None
        self._fitted = False

    def _compute_group_temporal_stats(self, X, groups):
        '''
        Compute per-group temporal statistics to assess dynamic feature variance.
        Returns a dict mapping group_id -> per-feature temporal variance.
        Used to inform fold splits and validate group separation.
        '''
        ...
        for g in unique_groups:
            ...
            group_stats[int(g)] = {
                'n_obs': int(mask.sum()),
                'feat_var_mean': float(np.nanmean(np.var(X_g, axis=0))),
                'feat_var_max': float(np.nanmax(np.var(X_g, axis=0))),
            }
        return group_stats
    ...
    def fit(self, X, Y, groups=None, X_column_names=None, do_oob_cv=True):
        ...
        # Compute per-group temporal statistics to assess dynamic feature variance
        if self._groups is not None and self.verbose > 0:
            self.group_temporal_stats_ = self._compute_group_temporal_stats(X, self._groups)
            n_groups = len(self.group_temporal_stats_)
            all_vars = [v['feat_var_mean'] for v in self.group_temporal_stats_.values()]
            print(f'Per-group temporal stats: {n_groups} groups, '
                  f'mean within-group variance={np.mean(all_vars):.4f}')
        ...
        if self.verbose > 0:
            ...
            print(f'  Total rows: {n_samples:,} | Groups: {len(np.unique(self._groups))}')
            print(f'  Memory-efficient fold iteration (no full data copy per fold)')
        ...
        def _fit_fold(fold_idx, train_idx, test_idx, seed):
            try:
                ...
                # Memory-efficient: pass index slices, not copies of large arrays
                est.fit(X[train_idx], Y[train_idx])
                preds = est.predict(X[test_idx])

                if self.verbose > 1:
                    n_test_groups = len(np.unique(self._groups[test_idx]))
                    print(f'  Fold {fold_idx+1}/{n_splits}: train={len(train_idx):,} '
                          f'test={len(test_idx):,} ({n_test_groups} groups)')
        ...
        if self.verbose > 0:
            print(f'OOB R2 (mean): {self.oob_r2_mean_:.4f}')
    ...
    def print_oob_summary(self):
        ...
        print(f'Overall OOB R2 (mean across targets): {self.oob_r2_mean_:.4f}')
...
def decorrelate_variance_aware(df, group_name, threshold=0.70, protect_cols=None, verbose=True):
    '''
    Variance-aware hierarchical decorrelation for streamflow predictors.

    Replaces simple threshold-based removal with variance-informed selection:
    when two features are highly correlated, the one with lower temporal variance
    is removed, preserving features most informative for streamflow dynamics.
    Columns listed in protect_cols are never dropped (used for dynamic features).
    '''
    ...
    for col in upper.columns:
        if col in to_drop or col in protect_cols:
            continue
        high_corr_with = upper.index[upper[col] > threshold].tolist()
        for other_col in high_corr_with:
            if other_col in to_drop:
                continue
            if other_col in protect_cols:
                # Protected column: drop the current col instead
                to_drop.add(col)
                break
            # Keep the feature with higher variance (more informative for streamflow)
    ...
    if verbose:
        print(f' {group_name:20s}: {len(df.columns):3d} -> {len(kept):3d} (rho > {threshold:.2f}, variance-aware)')
...
# Variance-aware decorrelation of static features
# Dynamic features are always kept (passed as protect_cols)
static_indices = [i for i, col in enumerate(all_cols) if col in static_present]
X_static_sample = X_sample[:, static_indices]
static_dec_df = decorrelate_variance_aware(
    pd.DataFrame(X_static_sample, columns=static_present),
    'Static',
    threshold=0.70,
    protect_cols=[],   # static only in this df; dynamic is handled separately
    verbose=True
)
...
if len(static_decorr) == 0:
    ...
    print(f'\nRunning RFECV on sampled static features: {X_static_sample_decorr.shape}')
    print(f'  Dynamic features ({len(dynamic_present)}) are always retained (not subject to RFECV)')
    ...
    print(f'  Dynamic features retained (all): {len(dynamic_present)}')
...
# Dynamic features are always retained: combine selected static with all dynamic
