...
#SBATCH -n 1 -c 16 -N 1
#SBATCH -t 24:00:00
#SBATCH -o /nfs/roberts/scratch/pi_ga254/ga254/stdout/sc31_LSTM_UNIFIED_FIXED.%A_%a.out
#SBATCH -e /nfs/roberts/scratch/pi_ga254/ga254/stderr/sc31_LSTM_UNIFIED_FIXED.%A_%a.err
#SBATCH --job-name=sc31_LSTM_UNIFIED_FIXED
#SBATCH --array=500
#SBATCH --mem=120G
...
print("\n" + "="*100)
print("SC31: UNIFIED LSTM PIPELINE v2 - FIXED COLUMN HANDLING")
print("="*100)
...
print(f"\nX columns ({len(X.columns)}): {list(X.columns)}")
...
# Create derived features (CORRECTLY!)
print("Creating derived features...")
X_train['ppt0_area'] = X_train['ppt0'].astype('float32') / X_train['accumulation'].astype('float32')
X_train['tmin0_area'] = X_train['tmin0'].astype('float32') / X_train['accumulation'].astype('float32')
X_train['soil0_area'] = X_train['soil0'].astype('float32') / X_train['accumulation'].astype('float32')
X_train['GRWLw_area'] = X_train['GRWLw'].astype('float32') / X_train['accumulation'].astype('float32')

X_test['ppt0_area'] = X_test['ppt0'].astype('float32') / X_test['accumulation'].astype('float32')
X_test['tmin0_area'] = X_test['tmin0'].astype('float32') / X_test['accumulation'].astype('float32')
X_test['soil0_area'] = X_test['soil0'].astype('float32') / X_test['accumulation'].astype('float32')
X_test['GRWLw_area'] = X_test['GRWLw'].astype('float32') / X_test['accumulation'].astype('float32')
    ...
    for q_var in q_cols:
        ...
        for lag in range(4):
            try:
                ...
                pass
        if max_rho > -np.inf:
            results.append({'Variable': d_var, 'ρ_max': max_rho})
    return results
...
print(f"  KEEP {len(dynamic_keep)}: {', '.join(dynamic_keep)}")
print(f"  REMOVE {len(dynamic_present) - len(dynamic_keep)}")
...
all_spatial_results = Parallel(n_jobs=NCPU)(delayed(compute_spatial_corr)(var) for var in static_2a_keep)
...
print(f"  KEEP {len(static_2b_keep)} variables (|ρ| ≥ {RHO_SPATIAL_THRESHOLD})")
...
print(f"  REMOVED {len(removed_vars)} variables → {len(static_final)} kept")

# Also add derived features to dynamic
dynamic_keep_with_derived = dynamic_keep + ['ppt0_area', 'tmin0_area', 'soil0_area', 'GRWLw_area']

print(f"\n{'='*60}")
print("FINAL FEATURE SELECTION")
print(f"{'='*60}")
print(f"Dynamic: {len(dynamic_keep_with_derived)} variables")
print(f"  {', '.join(dynamic_keep_with_derived)}")
print(f"Static: {len(static_final)} variables")
print(f"  {', '.join(sorted(static_final)[:15])}..." if len(static_final) > 15 else f"  {', '.join(sorted(static_final))}")
...
report_fs.add_section('DYNAMIC FEATURES', level=2)
report_fs.add_content(f'Selected: {len(dynamic_keep_with_derived)}')
for d in dynamic_keep_with_derived:
    report_fs.add_content(f'  {d}', indent=1)
report_fs.add_section('STATIC FEATURES', level=2)
report_fs.add_content(f'Selected: {len(static_final)} (from {len(static_present)} original)')
for s in sorted(static_final)[:20]:
...
print(f"  Architecture: {n_dyn} dyn + {n_sta} sta → 128 LSTM × 2 → 256 → 11 quantiles")
...
report_lstm.add_section('Results', level=2)
report_lstm.add_content(f'TRAINING: r={train_metrics["r"][0]:.4f}, NSE={train_metrics["nse"][0]:.4f}, KGE={train_metrics["kge"][0]:.4f}')
report_lstm.add_content(f'TESTING:  r={test_metrics["r"][0]:.4f}, NSE={test_metrics["nse"][0]:.4f}, KGE={test_metrics["kge"][0]:.4f}')

report_lstm.add_section('Per-Quantile Performance', level=2)
