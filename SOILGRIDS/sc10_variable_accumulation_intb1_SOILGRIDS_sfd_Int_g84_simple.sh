...
#### for tif in /gpfs/gibbs/pi/hydro/hydro/dataproces/SOILGRIDS/*/*/*_WeAv_transGrow.vrt ; do for BOX in AF AU EUA GL NA NAO SA SAO SIO SPO ; do MEM=$(grep ^"$BOX " /gpfs/gibbs/pi/hydro/hydro/dataproces/SOILGRIDS2/mem_request_BOX.txt | awk '{ print $2}'); sbatch  --export=tif=$tif,BOX=$BOX --mem=${MEM} --job-name=sc10_SOILGRIDS_sfd_$(basename $tif _WeAv_transGrow.vrt)_$BOX.sh /gpfs/gibbs/pi/hydro/hydro/scripts/SOILGRIDS/sc10_variable_accumulation_intb1_SOILGRIDS_sfd_Int_g84_simple.sh; done; done
...
export  tifname=$(basename $tif _transGrow.vrt )
...
if [ ! -e $SOILGRIDSC/${dir}_acc_sfd/$yyyy/intb/${tifname}_${box}_acc_sfd_Int_g84.tif ] || [ ! -s $SOILGRIDSC/${dir}_acc_sfd/$yyyy/intb/${tifname}_${box}_acc_sfd_Int_g84.mm ]; then
...
ll /vast/palmer/scratch/sbsc/hydro/dataproces/SOILGRIDS/$DIR/*_acc_sfd/intb/*_${BOX}_acc_sfd_Int_g84.tif
done
echo " " 
done 
...
done > /gpfs/gibbs/pi/hydro/hydro/dataproces/SOILGRIDS/mem_request_BOX.txt 


/usr/bin/grass -f --text --tmp-project /vast/palmer/scratch/sbsc/hydro/dataproces/SOILGRIDS/silt/silt_acc_sfd/intb/silt_0-200cm_SIO_var.tif  <<'EOF'
r.external input=/vast/palmer/scratch/sbsc/hydro/dataproces/SOILGRIDS/silt/silt_acc_sfd/intb/silt_0-200cm_SIO_var.tif output=test
