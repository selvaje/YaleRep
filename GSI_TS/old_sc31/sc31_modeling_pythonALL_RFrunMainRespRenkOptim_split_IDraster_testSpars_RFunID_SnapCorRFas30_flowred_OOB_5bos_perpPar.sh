...
class ClusteredRandomForestRegressor(BaseEstimator, RegressorMixin):
	...
	groups = np.asarray(groups).ravel()  # Ensure 1D
    ...
    def fit(self, X, y):
        ...
        self.estimators_ = []
        for s in seeds:
            self.estimators_.append(self._fit_single_tree(X_noID, y, self.groups_, s))
    ...
    def predict(self, X):
        # Drop IDr if present
        if X.shape == self.estimators_.n_features_in_ + 1:
            X_noID = X[:, 1:]
        else:
            X_noID = X
        # Improved: use joblib.Parallel for prediction (could also use ThreadPoolExecutor)
        from joblib import Parallel, delayed
        preds = Parallel(n_jobs=self.n_jobs)(
            delayed(self._predict_single_tree)(est, X_noID) for est in self.estimators_
        )
