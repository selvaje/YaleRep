...
y_target_cols = ['QMIN', 'Q10', 'Q20', 'Q30', 'Q40', 'Q50', 'Q60', 'Q70', 'Q80', 'Q90', 'QMAX']

print('\n--- Missing Values ---')
x_missing = X.isnull().sum()
y_missing = Y.isnull().sum()
x_missing_total = int(x_missing.sum())
y_missing_total = int(y_missing.sum())
print(f'X missing values: {x_missing_total} ({100*x_missing_total/X.size:.3f}%)')
print(f'Y missing values: {y_missing_total} ({100*y_missing_total/Y.size:.3f}%)')
if x_missing_total > 0:
    print('X columns with missing values:')
    print(x_missing[x_missing > 0].to_string())
if y_missing_total > 0:
    print('Y columns with missing values:')
    print(y_missing[y_missing > 0].to_string())

print('\n--- Y Target Descriptive Statistics ---')
y_stats = Y[y_target_cols].describe(percentiles=[0.01, 0.05, 0.25, 0.50, 0.75, 0.95, 0.99])
print(y_stats.to_string())

print('\n--- Y Skewness & Kurtosis ---')
y_skew = Y[y_target_cols].skew()
y_kurt = Y[y_target_cols].kurtosis()
y_dist_df = pd.DataFrame({'skewness': y_skew, 'kurtosis': y_kurt})
print(y_dist_df.to_string())

print('\n--- X Dynamic Feature Descriptive Statistics ---')
dyn_present_x = [c for c in dynamic_var if c in X.columns]
if dyn_present_x:
    x_dyn_stats = X[dyn_present_x].describe(percentiles=[0.05, 0.25, 0.50, 0.75, 0.95])
    print(x_dyn_stats.to_string())

print('\n--- X Static Feature Descriptive Statistics (variance summary) ---')
stat_present_x = [c for c in static_var if c in X.columns]
if stat_present_x:
    x_stat_var = X[stat_present_x].var().sort_values(ascending=False)
    print('Top 20 static features by variance:')
    print(x_stat_var.head(20).to_string())
    print('Bottom 10 static features by variance (potential near-zero variance):')
    print(x_stat_var.tail(10).to_string())

print('\n--- Temporal Patterns ---')
if 'YYYY' in X.columns and 'MM' in X.columns:
    year_counts = X['YYYY'].value_counts().sort_index()
    month_counts = X['MM'].value_counts().sort_index()
    n_stations = X['IDr'].nunique()
    obs_per_station = X.groupby('IDr').size()
    print(f'Unique stations (IDr): {n_stations}')
    print(f'Year range: {int(X["YYYY"].min())} - {int(X["YYYY"].max())}')
    print(f'Observations per station - mean: {obs_per_station.mean():.1f}, '
          f'median: {obs_per_station.median():.1f}, '
          f'min: {int(obs_per_station.min())}, max: {int(obs_per_station.max())}')
    print(f'Month distribution:')
    print(month_counts.to_string())

print('\n--- Y Temporal Pattern (mean Q50 by month) ---')
if 'MM' in Y.columns and 'Q50' in Y.columns:
    y_monthly = Y.groupby('MM')['Q50'].agg(['mean', 'std', 'count'])
    print(y_monthly.to_string())

print('\n--- X-Y Correlation (dynamic vars vs Q50) ---')
if dyn_present_x and 'Q50' in Y.columns:
    xy_corr_df = X[dyn_present_x].copy()
    xy_corr_df['Q50'] = Y['Q50'].values
    xy_corr = xy_corr_df.corr(method='spearman')['Q50'].drop('Q50').sort_values(key=abs, ascending=False)
    print('Spearman correlation of dynamic predictors with Q50:')
    print(xy_corr.to_string())

del x_missing, y_missing, y_stats, y_skew, y_kurt, y_dist_df
if dyn_present_x:
    del x_dyn_stats, xy_corr_df, xy_corr
gc.collect()
...
class GroupAwareMultiOutput(BaseEstimator, RegressorMixin):
    '''Multi-output regressor with group-aware OOB CV.

    Optimised for large datasets (11M+ rows):
    - GroupKFold OOB CV avoids temporal data leakage between stations
    - Chunked prediction limits peak memory use during inference
    - Per-group temporal statistics added as auxiliary features
      when temporal_features=True to capture within-station seasonality
    '''

    def __init__(self, base_estimator, n_cv_folds=5, n_jobs=1, inner_n_jobs=1,
                 random_state=24, oob_metric='r2', verbose=0,
                 predict_chunk_size=200000, temporal_features=True):
    ...
    def fit(self, X, Y, groups=None, X_column_names=None, do_oob_cv=True):
        ...
        # Stratify splits so every fold covers the full temporal range of groups
        unique_groups = np.unique(self._groups)
        n_splits = min(self.n_cv_folds, len(unique_groups))
        ...
        if self.verbose > 0:
            print(f'Running GroupKFold OOB CV with {n_splits} splits '
                  f'({len(unique_groups)} unique stations, {n_samples} rows)...')
    ...
    def predict(self, X):
        '''Predict in chunks to keep memory bounded for 11M+ row inference.'''
        if len(self.models_) == 0 or not self._fitted:
            raise ValueError('Model not fitted.')
        X = np.asarray(X, dtype=np.float32)
        n = X.shape[0]
        chunk = self.predict_chunk_size
        if n <= chunk:
            return self.models_[0].predict(X)
        parts = []
        for start in range(0, n, chunk):
            parts.append(self.models_[0].predict(X[start:start + chunk]))
...
def decorrelate_group_smart(df, group_name, threshold=0.70, verbose=True):
    ...
    lower variance is dropped, preserving the feature that carries
    more unique information and is harder to reconstruct from the other.
    '''
    ...
    for col in upper.columns:
        ...
        for other_col in high_corr_with:
            if other_col in to_drop:
                continue
            # Keep the feature with higher variance; drop the lower-variance one
            if variances[col] >= variances[other_col]:
                drop_col = other_col
                keep_col = col
            else:
                drop_col = col
            ...
            rho_val = float(upper.loc[other_col, col])
            drop_reasons[drop_col] = (keep_col, rho_val,
                                       float(variances[drop_col]),
                                       float(variances[keep_col]))

    kept = [c for c in df.columns if c not in to_drop]
    if verbose:
        print(f' {group_name:20s}: {len(df.columns):3d} -> {len(kept):3d} vars kept '
              f'(rho > {threshold:.2f}, variance-preserving)')
        if drop_reasons:
            print(f'   Dropped features (lower-variance partner removed):')
            for drop_col, (keep_col, rho_val, drop_var, keep_var) in list(drop_reasons.items())[:10]:
                print(f'     drop={drop_col:30s}  kept={keep_col:30s}  '
                      f'rho={rho_val:.3f}  var_drop={drop_var:.4f}  var_keep={keep_var:.4f}')
            if len(drop_reasons) > 10:
                print(f'     ... and {len(drop_reasons)-10} more dropped features')

    del corr, upper, variances
    gc.collect()
...
# Sample data for faster feature selection (preserves full temporal resolution)
...
if len(static_decorr) == 0:
    ...
    n_static = len(static_decorr)
    # Conservative step: remove ~10% per round to preserve more features
    RFECV_STEP_RATIO = 0.10
    RFECV_MIN_RATIO = 0.30
    RFECV_MIN_FLOOR = 5
    RFECV_CV_FOLDS = 5
    step_size = max(2, int(n_static * RFECV_STEP_RATIO))
    # Keep at least 30% of features or 5, whichever is larger
    min_features = max(RFECV_MIN_FLOOR, int(n_static * RFECV_MIN_RATIO))

    # ExtraTrees uses more estimators and deeper trees than the main RF
    # because feature selection benefits from lower bias at the cost of speed
    RFECV_N_EST = 80
    RFECV_MAX_DEPTH = 20
    RFECV_MIN_LEAF = 3
    RFECV_MIN_SPLIT = 6
    ...
    selector_fast = RFECV(
        estimator=ExtraTreesRegressor(
            n_estimators=RFECV_N_EST,
            max_depth=RFECV_MAX_DEPTH,
            n_jobs=1,
            random_state=24,
            min_samples_leaf=RFECV_MIN_LEAF,
            min_samples_split=RFECV_MIN_SPLIT
        ),
        step=step_size,
    ...
    rfecv_results_df = pd.DataFrame({
        ...
        'Rank': rfecv_rankings,
        'Variance': X_static_sample_decorr.var(axis=0)
    })
    ...
    print(f'  CV folds used: {RFECV_CV_FOLDS} (GroupKFold by station)')
    print(f'  RFECV step size: {step_size} ({100*step_size/n_static:.1f}% of features per step)')
    print(f'  Min features enforced: {min_features}')

    selected_features_table = rfecv_results_df[rfecv_results_df['Selected']==True][['Feature', 'Rank', 'Survival_Score', 'Variance', 'Interpretation']]
    print('\nSELECTED FEATURES (Rank = 1):')
    print('-'*130)
    print(selected_features_table.to_string(index=False))

    eliminated_features_table = rfecv_results_df[rfecv_results_df['Selected']==False].head(10)[['Feature', 'Rank', 'Survival_Score', 'Variance', 'Interpretation']]
