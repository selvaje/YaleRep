...
#SBATCH -o /vast/palmer/scratch/sbsc/ga254/stdout/sc10_TERRA_sfd_Int_g84_s.sh.%A_%a.out
#SBATCH -e /vast/palmer/scratch/sbsc/ga254/stderr/sc10_TERRA_sfd_Int_g84_s.sh.%A_%a.err
#SBATCH --array=306

#### AF AU EUA GL NA NAO SA SAO SIO SPO
#### /gpfs/gibbs/pi/hydro/hydro/dataproces/TERRA/

#### for BOX in EUA NA AU AF SPO SA SIO GL SAO NAO ; do MEM=$(grep ^"$BOX " /gpfs/gibbs/pi/hydro/hydro/dataproces/TERRA/mem_request_BOX.txt | awk '{ print $2}'); sbatch  --export=BOX=$BOX,dir=soil --mem=${MEM} --job-name=sc10_TERRA_sfd_$BOX.sh /gpfs/gibbs/pi/hydro/hydro/scripts/TERRA/sc10_variable_accumulation_intb1_TERRA_sfd_Int_g84_simple.sh ; done
...
export  yyyy=$(awk -v AR=$SLURM_ARRAY_TASK_ID  '{ if(NR==AR)  print $2 }' /gpfs/gibbs/pi/hydro/hydro/dataproces/TERRA/nr_year_month_list.txt )
export  mm=$(awk -v AR=$SLURM_ARRAY_TASK_ID '{ if(NR==AR)  print $3 }' /gpfs/gibbs/pi/hydro/hydro/dataproces/TERRA/nr_year_month_list.txt )

export  tif=/gpfs/gibbs/pi/hydro/hydro/dataproces/TERRA/${dir}/${dir}_${yyyy}_${mm}.tif
...
sbatch  --job-name=sc11_tiling20d_TERRA_${dir}_${yyyy}_${mm}.sh   --exclude=r909u13n03  --export=dir=$dir,yyyy=$yyyy,mm=$mm /gpfs/gibbs/pi/hydro/hydro/scripts/TERRA/sc11_tiling20d_TERRA_sfd_Int_g84.sh
...
for line in $( cat /tmp/list.txt  ) ; do
    ...
    dir=tmin 
    nr=$(grep "$yyyy $mm"    /gpfs/gibbs/pi/hydro/hydro/dataproces/TERRA/nr_year_month_list.txt | cut -d " "  -f 1)
echo -n ${nr},
done

    
for BOX in EUA NA AU AF SPO SA SIO GL SAO NAO ; do
    MEM=$(grep ^"$BOX " /gpfs/gibbs/pi/hydro/hydro/dataproces/TERRA/mem_request_BOX.txt | awk '{ print $2}')
    echo sbatch   --export=BOX=$BOX,dir=tmin  --mem=${MEM} --job-name=sc10_TERRA_sfd_$BOX.sh /gpfs/gibbs/pi/hydro/hydro/scripts/TERRA/sc10_variable_accumulation_intb1_TERRA_sfd_Int_g84_simple.sh
done

############ final check
for yyyy in {1958..2018} ; do
    for mm in 01 02 03 04 05 06 07 08 09 10 11 12 ; do
    ll /gpfs/gibbs/pi/hydro/hydro/dataproces/TERRA/tmin_acc_sfd/$yyyy/tmin_${yyyy}_${mm}_sfd_10p.tif
    done
done  | grep directory 
