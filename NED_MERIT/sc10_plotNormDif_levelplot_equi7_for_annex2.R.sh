...
if(dir == "elev_dif" ) { des="Elevation diff.  MERIT - 3DEP" ; max=round(elev_dif@data@max) ; min=round(elev_dif@data@min) ; at=c( min,0,max); labels=c(min,0 ,  max ) ; letter="d" }
