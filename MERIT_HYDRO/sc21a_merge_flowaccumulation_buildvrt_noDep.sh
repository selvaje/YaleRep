...
#### sbatch  /gpfs/gibbs/pi/hydro/hydro/scripts/MERIT_HYDRO/sc21a_merge_flowaccumulation_buildvrt_noDep.sh

#### sbatch  --dependency=afterany:$(   squeue -u $USER -o "%.9F %.80j" | grep   sc20_build_dem_location_HandsTilesBASINS  |  awk '{ printf ("%i:" , $1)} END {gsub(":","") ; print $1 }'  )    /gpfs/gibbs/pi/hydro/hydro/scripts/MERIT_HYDRO/sc21a_merge_flowaccumulation_buildvrt_noDep.sh
...
gdalbuildvrt -overwrite -srcnodata -9999999 -vrtnodata -9999999 $MERIT/flow_noDep_tiles_intb1/all_tif_dis.vrt $(for ID in $(seq 1 59) ; do ls $MERIT/flow_noDep_tiles_intb1/flow_*${ID}.tif ; done) 
else
sleep 100
fi

gdal_translate -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -co TILED=YES  -a_nodata -9999999 -ot Float32 -projwin $ulx $uly $lrx $lry $MERIT/flow_noDep_tiles_intb1/all_tif_dis.vrt  $RAM/flow_${tile}.tif 
...
pksetmask -co COMPRESS=DEFLATE -co ZLEVEL=9 -m $RAM/flow_${tile}.tif  -msknodata -9999999 -nodata  -9999999 -i $RAM/flow_${tile}_pos.tif -o $MERIT/flow_noDep_tiles/flow_${tile}_pos.tif
rm -f $RAM/flow_${tile}_pos.tif

# pkfilter -nodata -9999999  -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -co TILED=YES  -ot Float32 -of GTiff -dx 4 -dy 4 -d 4 -f mean -i  $RAM/flow_$tile.tif -o $MERIT/flow_tiles/flow_${tile}_4p.tif
# pkfilter -nodata -9999999  -co COMPRESS=DEFLATE -co ZLEVEL=9 -co INTERLEAVE=BAND -co TILED=YES  -ot Float32 -of GTiff -dx 10 -dy 10 -d 10 -f mean -i $RAM/flow_$tile.tif -o $MERIT/flow_tiles/flow_${tile}_10p.tif 
mv   $RAM/flow_${tile}.tif   $MERIT/flow_noDep_tiles/flow_${tile}.tif 
fi 
