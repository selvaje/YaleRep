...
#SBATCH -n 1 -c 2 -N 1
#SBATCH -t 12:00:00       # 
#SBATCH -o /gpfs/gibbs/pi/hydro/hydro/stdout1/sc05_GEDI_process_all_followDownload.sh.%J.out
#SBATCH -e /gpfs/gibbs/pi/hydro/hydro/stderr1/sc05_GEDI_process_all_followDownload.sh.%J.err
#SBATCH --job-name=sc05_GEDI_process_all_followDownload.sh
#SBATCH --mem=70G
...
#### cehck for ram usage 
### for ID in $(ls /gpfs/gibbs/pi/hydro/hydro/stderr1/sc05_GEDI_process_all_followDownload.sh.*.err | awk -F . '{ print $3  }' ); do seff $ID | grep "Memory Utilized" ; done  | sort -k 2,2 -g
...
LASTDATE=$(grep  $( echo $DATE | awk -F . '{ print $1 "."  $2  }'   )     /gpfs/gibbs/pi/hydro/hydro/dataproces/GEDI/date_2019_2020.txt | tail -1)

if [ "$DATE" =  "$LASTDATE" ] ; then
MONTH=$(grep  $DATE   /gpfs/gibbs/pi/hydro/hydro/dataproces/GEDI/date_2019_2020.txt  |  awk -F . '{ print $1 "."  $2  }'   )
echo current $MONTH
MONTHPLUS=$(grep  $MONTH -A 1  <(awk -F . '{ print $1 "."  $2  }'   /gpfs/gibbs/pi/hydro/hydro/dataproces/GEDI/date_2019_2020.txt  | sort | uniq ) |  tail -1 )
echo next $MONTHPLUS
narray=$(grep $MONTHPLUS  /gpfs/gibbs/pi/hydro/hydro/dataproces/GEDI/date_2019_2020.txt | wc -l )  
sbatch --export=YYYYMM=$MONTHPLUS   --array=1-$narray /gpfs/gibbs/pi/hydro/hydro/scripts/GEDI/sc02_GEDI_download_all.sh
fi
