...
class GroupAwareMultiOutput(BaseEstimator, RegressorMixin):
    ...
    - Better calibration of OOB predictions
    """
    ...
    def fit(self, X, Y, groups=None, X_column_names=None, do_oob_cv=True):
        ...
        np.random.RandomState(self.random_state).shuffle(group_indices)
        ...
        def _fit_fold(fold_idx, test_group_indices, seed):
            try:
                ...
                if self.verbose > 1:
                    print(f'  Fold {fold_idx+1}/{n_splits}: train={len(train_idx)}, test={len(test_idx)}, R²≈{self._quick_r2(Y[test_idx], preds):.4f}')
...
def decorrelate_group_improved(df, group_name, threshold=0.85, max_features_remove=0.40, verbose=True):
    """
    ...
    for i, feat_idx in enumerate(var_ranked):
