...
# ============================================================================
# STATIC/DYNAMIC VARIABLE DEFINITIONS (MOVED UP - DEFINED FIRST)
# ============================================================================
...
np.savetxt(f'../predict_splitting_red/stationID_x_y_valueALL_predictors_YTrainN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_2RF.txt',Y_train.values, delimiter=' ', fmt=fmt_Y, header=Y_column_names_str, comments='')
np.savetxt(f'../predict_splitting_red/stationID_x_y_valueALL_predictors_YTestN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_2RF.txt',Y_test.values, delimiter=' ', fmt=fmt_Y, header=Y_column_names_str, comments='')
...
# ✓ FIX: Extract MM, IDr BEFORE deleting X_train!
...
# ✓ NOW delete (after extracting what we need)
...
# ============================================================================
# FAST STATION-LEVEL FEATURE SELECTION
# ============================================================================

print('\n===== FAST STATION-LEVEL FEATURE SELECTION =====')
print('Building station-level matrices for faster feature selection...')
...
if len(static_present) == 0:
    ...
    # Build station-MONTH level aggregates (ONE row per station per month)
    station_df_full = pd.DataFrame(X_train_np, columns=all_cols)
    station_df_full['IDr'] = groups_train
    station_df_full['MM'] = X_train_mm  # ✓ Use extracted variable, not X_train['MM']
    
    # Get ONE representative row per station-month for static features
    station_summary = station_df_full.groupby(['IDr', 'MM'])[static_present].first().reset_index(drop=False)
    # Keep IDr and MM for grouping later
    station_idr_mm = station_summary[['IDr', 'MM']].copy()
    station_summary = station_summary[static_present].reset_index(drop=True)
    
    # For targets, use station-MONTH level MEAN streamflow quantiles
    ...
    Y_train_station['MM'] = X_train_mm  # ✓ Use extracted variable, not X_train['MM']
    ...
    if len(static_decorr) == 0:
        ...
        print(f'\nRunning FAST GroupAwareRFECV on station-month level data: {X_static_station.shape}')
        ...
        selector_fast = GroupAwareRFECV(
            ...
            step=max(5, int(len(static_decorr) * 0.35)),
            min_features_to_select=max(3, int(len(static_decorr) * 0.1)),
            scoring='r2',
            n_jobs=8,
            verbose=1
        )
        ...
        # ✓ KEY FIX: Create groups so all months of SAME station stay together in CV
        # station_idr_mm contains IDr values - use that as groups
        station_groups = station_idr_mm['IDr'].values.astype('int32')
        
        selector_fast.fit(
            X_static_station,
            Y_train_station_mean.values.astype('float32'),
            groups=station_groups,  # ✓ Groups by IDr (all months of same station together)
        ...
        rfecv_results_df.to_csv(f'../predict_importance_red/stationID_x_y_valueALL_predictors_RFECVrankingN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_static.txt',
                                index=False, sep=' ')
        print(f'\n✓ Full RFECV ranking saved to: ../predict_importance_red/stationID_x_y_valueALL_predictors_RFECVrankingN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_static.txt')
