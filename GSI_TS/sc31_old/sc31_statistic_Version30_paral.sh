...
def compute_predictor_importance_parallel(var_name, df_data):
    if var_name in df_data.columns:
        vd = df_data[[var_name, 'Q50']].dropna()
        if len(vd) > 2:
            pr, pp = pearsonr(vd[var_name], vd['Q50'])
            return {'Predictor_Type': 'Static', 'Predictor': var_name, 'Pearson_r': abs(pr), 'Num_Samples': len(vd)}
    return None

def compute_dynamic_importance_parallel(var_name, df_data):
    if var_name in df_data.columns:
        vd = df_data[[var_name, 'Q50']].dropna()
        if len(vd) > 2:
            pr, pp = pearsonr(vd[var_name], vd['Q50'])
            return {'Predictor_Type': 'Dynamic', 'Predictor': var_name, 'Pearson_r': abs(pr), 'Num_Samples': len(vd)}
    return None

def compute_autocorr_parallel(station_id, df_data):
    sd = df_data[df_data['IDr'] == station_id].sort_values(['YYYY', 'MM'])
    if len(sd) > 12:
        qv = sd['Q50'].dropna().values
        if len(qv) > 1:
            return np.corrcoef(qv[:-1], qv[1:])[0, 1]
    return None

def compute_lag_corr_parallel(station_id, df_data):
    sd = df_data[df_data['IDr'] == station_id].sort_values(['YYYY', 'MM'])
    if len(sd) > 24:
        qv = sd['Q50'].dropna().values
        if len(qv) > 2:
...
print('Computing predictor importance (PARALLEL)...')
static_results = Parallel(n_jobs=-1)(delayed(compute_predictor_importance_parallel)(var, df_analysis) for var in static_var)
static_results = [r for r in static_results if r is not None]

dynamic_results = Parallel(n_jobs=-1)(delayed(compute_dynamic_importance_parallel)(var, df_analysis) for var in dynamic_var)
dynamic_results = [r for r in dynamic_results if r is not None]

static_importance = static_results + dynamic_results
...
autocorr_results = Parallel(n_jobs=-1)(delayed(compute_autocorr_parallel)(sid, df_analysis) for sid in sample_stations)
autocorr_results = [r for r in autocorr_results if r is not None]
...
print('Computing lag correlations (PARALLEL)...')
sample_stations_lag = df_analysis['IDr'].unique()[:min(30, len(df_analysis['IDr'].unique()))]
lag_corr = Parallel(n_jobs=-1)(delayed(compute_lag_corr_parallel)(sid, df_analysis) for sid in sample_stations_lag)
lag_corr = [r for r in lag_corr if r is not None]
