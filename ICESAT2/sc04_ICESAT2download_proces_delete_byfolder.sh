...
#SBATCH -o /gpfs/scratch60/fas/sbsc/ga254/stdout/sc04_ICESAT2download_proces_delete_byfolder.sh.%A_%a.out
#SBATCH -e /gpfs/scratch60/fas/sbsc/ga254/stderr/sc04_ICESAT2download_proces_delete_byfolder.sh.%A_%a.err
#SBATCH --mem=5G
#SBATCH --array=1-796%1
...
###### sbatch /gpfs/gibbs/pi/hydro/hydro/scripts/ICESAT2/sc04_ICESAT2download_proces_delete_byfolder.sh
...
wget --load-cookies ~/.urs_cookies --save-cookies ~/.urs_cookies --keep-session-cookies --no-check-certificate --auth-no-challenge=on -r --reject "index.html*"  -e robots=off  --user=toby_tang --password=Tzp19910908 -c  -np -nH  --cut-dirs 3  -P $OUP_H5     https://n5eil01u.ecs.nsidc.org/ATLAS/ATL08.004/${DATE}.zip   --progress=bar:force:noscroll
