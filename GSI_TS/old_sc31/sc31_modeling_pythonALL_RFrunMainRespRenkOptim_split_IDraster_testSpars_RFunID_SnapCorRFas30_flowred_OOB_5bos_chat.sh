...
class ClusteredRandomForestRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        #     Must have IDr as the first column.
        # y : ndarray, shape (n_samples, n_outputs)

        rng = check_random_state(self.random_state)

        # Extract groups and predictors
        ...
        for i in range(self.n_estimators):
            ...
            mask = np.isin(self.groups_, sampled_groups)
    ...
    def predict(self, X):
        # Predicts ensuring non-negative outputs.
        # Automatically drops IDr if present
        # Drop IDr if user passed it
        if X.shape[1] == self.estimators_[0].n_features_in_ + 1:
            X = X[:, 1:]

        preds = np.stack([est.predict(X) for est in self.estimators_], axis=0)
        y_pred = np.mean(preds, axis=0)
        return np.maximum(y_pred, 0)  # enforce non-negativity
