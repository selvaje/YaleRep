...
def decorrelate_by_spearman_station_level(X_np, groups, col_names, threshold=0.85):
    ...
    print(f'Static station-level decorrelation: start {len(cols)} keep {len(keep)} drop {len(drop)} thr {threshold}')
    return keep
...
def filter_static_by_response_spearman_obs_level(X_np, Y_np, col_names, threshold=0.85):
    ...
    print(f'Response-vs-static filter: start {len(col_names)} keep {len(keep)} drop {len(drop)} thr {threshold}')
    return keep

def group_oob_r2(et_model, X_np, Y_np, groups):
    ...
    return float(np.mean(r2s)), np.array(r2s, dtype=float)
...
X_static_np = X_train_np[:, static_idx] if static_idx.size > 0 else np.empty((X_train_np.shape[0], 0), dtype=float)
X_dynamic_np = X_train_np[:, dynamic_idx] if dynamic_idx.size > 0 else np.empty((X_train_np.shape[0], 0), dtype=float)

# 1) static decorrelation at station level
...
# 2) response vs static decorrelation at observation level
X_static_keep1_np = pd.DataFrame(X_static_np, columns=static_cols).loc[:, static_keep1].to_numpy() if len(static_keep1) > 0 else np.empty((X_train_np.shape[0], 0), dtype=float)
...
print(f'Selected static vars after both steps: {len(selected_static)}')
...
X_train_dyn = X_train_np[:, dynamic_idx] if dynamic_idx.size > 0 else np.empty((X_train_np.shape[0], 0), dtype=float)
X_test_dyn = X_test_np[:, dynamic_idx] if dynamic_idx.size > 0 else np.empty((X_test_np.shape[0], 0), dtype=float)

# 3) ExtraTreesRegressor_SD
...
oob_r2_sd_all, oob_r2_sd_cols = group_oob_r2(ET_SD, X_train_SD, Y_train_np, groups_train)
print(f'ExtraTreesRegressor_SD group OOB R2 mean: {oob_r2_sd_all}')
print(f'ExtraTreesRegressor_SD group OOB R2 cols: {oob_r2_sd_cols}')

# 4) ExtraTreesRegressor_D
...
oob_r2_d_all, oob_r2_d_cols = group_oob_r2(ET_D, X_train_dyn, Y_train_np, groups_train)
print(f'ExtraTreesRegressor_D group OOB R2 mean: {oob_r2_d_all}')
print(f'ExtraTreesRegressor_D group OOB R2 cols: {oob_r2_d_cols}')

# 5) Use ExtraTreesRegressor_SD for downstream error matrix/predictions
class _WrappedModel:
...
print(f'Using ExtraTreesRegressor_SD for predictions with {X_train_selected.shape[1]} features.')
