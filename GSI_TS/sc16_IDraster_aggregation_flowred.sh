...
####   sbatch /gpfs/gibbs/pi/hydro/hydro/scripts/GSI_TS/sc16_IDraster_aggregation_flowred.sh
...
}' /tmp/$(basename $file)  >> $IN/snapFlow_txt_red/IDs_xsnap_ysnap_IDseg_IDrt_IDru_all.txt
rm -f  /tmp/$(basename $file)
counter=$( awk 'END { print $6 }' $IN/snapFlow_txt_red/IDs_xsnap_ysnap_IDseg_IDrt_IDru_all.txt)
done 
...
rm -f $IN/snapFlow_gpkg_red/ximpr_yimpr_IDs_all.gpkg
pkascii2ogr -a_srs epsg:4326 -f "GPKG" -x 0 -y 1 -n "IDstation"  -ot "Integer"   -i $IN/snapFlow_txt_red/ximpr_yimpr_IDs_all.txt   -o $IN/snapFlow_gpkg_red/ximpr_yimpr_IDs_all.gpkg
rm -f $IN/snapFlow_gpkg_red/xsnap_ysnap_IDs_IDr_all.gpkg 
pkascii2ogr -a_srs epsg:4326   -f "GPKG" -x 0 -y 1 -n "IDstation" -ot "Integer" -n "IDraster" -ot "Integer" -i $IN/snapFlow_txt_red/xsnap_ysnap_IDs_IDr_all.txt  -o $IN/snapFlow_gpkg_red/xsnap_ysnap_IDs_IDr_all.gpkg
...
rm -f $IN/snapFlow_gpkg_red/xsnap_ysnap_IDs_IDr_all_eqdist.gpkg
ogr2ogr   -t_srs EPSG:32662 $IN/snapFlow_gpkg_red/xsnap_ysnap_IDs_IDr_all_eqdist.gpkg  $IN/snapFlow_gpkg_red/xsnap_ysnap_IDs_IDr_all.gpkg  
# -skipfailures
### from this point on IDs = IDstation ; IDr = global uniq ID raster 

echo "IDs IDr Xcoord Ycoord" > $IN/snapFlow_txt_red/IDs_IDr_xcoord_ycoord.txt
paste -d " "  <(ogrinfo -al $IN/snapFlow_gpkg_red/xsnap_ysnap_IDs_IDr_all_eqdist.gpkg   | grep "IDstation (Integer)")  \
              <(ogrinfo -al $IN/snapFlow_gpkg_red/xsnap_ysnap_IDs_IDr_all_eqdist.gpkg   | grep "IDraster (Integer)")  \
              <(ogrinfo -al $IN/snapFlow_gpkg_red/xsnap_ysnap_IDs_IDr_all_eqdist.gpkg   | grep POINT ) \
    | sed  's/)/ /g' | sed  's/(/ /g' | awk '{ print $4,$8,$10,$11 }'  >> $IN/snapFlow_txt_red/IDs_IDr_xcoord_ycoord.txt
