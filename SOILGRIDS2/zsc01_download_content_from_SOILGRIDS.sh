...
#SBATCH -n 1 -c 1 -N 1
#SBATCH -t 24:00:00                                                                                                         
#SBATCH -o /vast/palmer/scratch/sbsc/sm3665/stdout/sc01_download_content_from_SOILGRIDS.sh.%A_%a.out
#SBATCH -e /vast/palmer/scratch/sbsc/sm3665/stderr/sc01_download_content_from_SOILGRIDS.sh.%A_%a.err
#SBATCH --mem=20G
#SBATCH --array=1-4%2
...
export depth=$depth ####check just if the variable are imported in the sbatch                                                                                             
export INPUT_URL=/vsicurl/https://files.isric.org/soilgrids/latest/data
export OUTPUT=/gpfs/gibbs/project/sbsc/sm3665/dataproces/SOILGRIDS2
...
if [ $SLURM_ARRAY_TASK_ID -eq 3 ]; then
    ...
    sbatch --export=var=$var,depth=$depth \
	   --dependency=afterany:${current_jobid} /vast/palmer/home.grace/sm3665/scripts/download/sc02_merge_tiles_from_SOILGRIDS.sh
fi
