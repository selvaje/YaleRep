...
# ---------------------------
# GroupAwareRandomForest2 (robust, parallel, OOB, importances)
# ---------------------------
class GroupAwareRandomForest2:
    ...
    def _fit_single_tree(self, X, y, groups, seed):
        
	# Worker function: builds local rng from seed, does group-aware bootstrap,
        # trains one DecisionTree, returns (tree, oob_idx_array).
        # This function NEVER returns None; in case of failure it returns a fallback tree.
        ...
        try:
            ...
            if groups is not None:
                ...
                # Sample groups with replacement (bootstrap groups)
                sampled_groups = rng.choice(unique_groups, size=len(unique_groups), replace=True)
                inbag_groups = np.unique(sampled_groups)

                # Build train indices by sampling rows within each in-bag group (with replacement)
                train_idx_list = []
                for g in inbag_groups:
                    idx_g = np.where(groups == g)[0]
                    # sample rows of that group with replacement
                    if idx_g.size > 0:
                        train_idx_g = rng.choice(idx_g, size=idx_g.size, replace=True)
                        train_idx_list.append(train_idx_g)
                if len(train_idx_list) == 0:
                    train_idx = np.array([], dtype=int)
                else:
                    train_idx = np.hstack(train_idx_list)

                # OOB groups are those not included at all
                oob_groups = np.setdiff1d(unique_groups, inbag_groups)
                if oob_groups.size > 0:
                    oob_idx = np.hstack([np.where(groups == g)[0] for g in oob_groups])
                else:
                    oob_idx = np.array([], dtype=int)
            else:
                # Regular row-level bootstrap
                train_idx = rng.choice(n_samples, size=n_samples, replace=True)
                oob_idx = np.setdiff1d(np.arange(n_samples), train_idx)

            # If train_idx somehow empty, fallback to a small random sample
            if train_idx.size == 0:
                train_idx = rng.choice(n_samples, size=min(10, n_samples), replace=False)
            ...
            # Fallback: create a tiny tree on a tiny subset so we don't return None.
            # Log minimal info by returning fallback tree and empty oob.
            try:
                n_samples = X.shape[0]
                take = min(10, n_samples)
                tree = GroupAwareDecisionTree(max_features=self.max_features)
                tree.fit(X[:take], y[:take])
                return (tree, np.array([], dtype=int))
            except Exception:
                # As a last resort, return a trivial tree trained on first row repeated
                tree = GroupAwareDecisionTree(max_features=self.max_features)
                tree.fit(np.repeat(X[:1], 2, axis=0), np.repeat(y[:1], 2, axis=0))
                return (tree, np.array([], dtype=int))

    def fit(self, X, y, groups=None):
        
        # X: 2D array (n_samples, n_features) - DO NOT include the grouping column IDr in X
        # y: 2D array (n_samples, n_outputs) OR 1D array (n_samples,)
        # groups: 1D array of group IDs (n_samples) or None
        ...
        # Create seeds for workers to avoid shared RNG
        rng_master = np.random.default_rng(self.random_state)
        seeds = rng_master.integers(0, 2**31 - 1, size=self.n_estimators)

        # Run parallel tree training
        results = Parallel(n_jobs=self.n_jobs)(
            delayed(self._fit_single_tree)(X, y, groups, int(seed))
            for seed in seeds
        )

        # Ensure results are tuples and filter any accidental None (shouldn't happen now)
        results = [res for res in results if (isinstance(res, tuple) and len(res) == 2)]
        if len(results) == 0:
            raise RuntimeError('All tree fittings failed; check inputs/parameters.')

        # Unpack
        self.estimators_, all_oob_idx = zip(*results)  # safe now

        # ---- OOB evaluation (group-aware) ----
        if self.oob_metric is not None:
            n_samples = X.shape[0]
            n_outputs = y.shape[1]
            preds_sum = np.zeros((n_samples, n_outputs), dtype=float)
            preds_count = np.zeros((n_samples, n_outputs), dtype=float)
            ...
            for (tree, oob_idx) in results:
                ...
                # align shapes if single-output
                if preds.ndim == 1:
                    preds = preds.reshape(-1, 1)
                preds_sum[oob_idx] += preds
                preds_count[oob_idx] += 1

            mask_rows = np.any(preds_count > 0, axis=1)
            if np.any(mask_rows):
                oob_preds = np.zeros((np.sum(mask_rows), n_outputs), dtype=float)
                # avoid division by zero
                valid_counts = preds_count[mask_rows]
                oob_preds = preds_sum[mask_rows] / np.where(valid_counts == 0, 1, valid_counts)

                if self.oob_metric == 'r2':
                    # global r2 (multioutput average) + per-output r2
                    try:
                        global_r2 = r2_score(y[mask_rows], oob_preds, multioutput='uniform_average')
                        per_output_r2 = [r2_score(y[mask_rows, j], oob_preds[:, j]) for j in range(n_outputs)]
        ...
        # If trees disagree in number of features, try to be robust
        try:
            importances = np.mean([tree.feature_importances_ for tree in self.estimators_], axis=0)
            self.feature_importances_ = importances
        except Exception:
            # if anything unexpected, set to zeros
    ...
    def predict(self, X):
        ...
        # all_preds shape: (n_trees, n_samples, n_outputs) or (n_trees, n_samples)
        if all_preds.ndim == 2:
