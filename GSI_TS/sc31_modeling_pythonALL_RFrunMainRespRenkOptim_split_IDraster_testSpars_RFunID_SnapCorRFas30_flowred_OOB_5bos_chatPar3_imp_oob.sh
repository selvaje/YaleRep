...
with parallel_backend('threading', n_jobs=self.n_jobs):
    results = Parallel()(delayed(self._fit_single_tree)(X_noID, y, self.groups_, s) for s in seeds)
...
class ClusteredRandomForestRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, 
                 n_estimators=100, 
                 max_features='sqrt', 
                 min_samples_leaf=1, 
    ...
    def _fit_single_tree(self, X_noID, y, groups, rng_seed):
        ...
        est = RandomForestRegressor(
            ...
            random_state=rng.randint(0, 1e9),
            **self.rf_kwargs
        )
        est.fit(X_noID[mask], y[mask])

        mask_oob = ~mask
        preds_oob = None
        if np.any(mask_oob):
            preds_oob = est.predict(X_noID[mask_oob])
            if preds_oob.ndim == 1:
                preds_oob = preds_oob.reshape(-1, 1)

        del rng, unique_groups, sampled_groups, mask
        gc.collect()
        return est, mask_oob, preds_oob
    ...
    def fit(self, X, y):
        ...
        # ensure y is 2D
        y = np.atleast_2d(y)
        if y.shape[0] == X_noID.shape[0] and y.shape[1] == 1:
            y = y.reshape(-1, 1)

        n_samples, n_targets = y.shape
        seeds = rng.randint(0, 1e9, size=self.n_estimators)

        results = Parallel(n_jobs=self.n_jobs)(
            delayed(self._fit_single_tree)(X_noID, y, self.groups_, s) for s in seeds
        )

        self.estimators_ = [r[0] for r in results]

        oob_preds = np.zeros((n_samples, n_targets))
        oob_counts = np.zeros(n_samples)

        for est, mask_oob, preds_oob in results:
            if preds_oob is not None:
                oob_preds[mask_oob] += preds_oob
                oob_counts[mask_oob] += 1

        mask_valid = oob_counts > 0
        if np.any(mask_valid):
            oob_preds[mask_valid] /= oob_counts[mask_valid][:, None]

        # --- compute OOB errors ---
        oob_errors = np.zeros(n_targets + 1)
        ...
        def compute_metric(y_true, y_pred, metric):
            if metric == 'mse':
                ...
                raise ValueError(f'Unsupported oob_metric: {metric}')

        if np.any(mask_valid):
            # general
            oob_errors[0] = compute_metric(
                y[mask_valid].ravel(), oob_preds[mask_valid].ravel(), self.oob_metric
            )
            # per target
            for j in range(n_targets):
                oob_errors[j+1] = compute_metric(
                    y[mask_valid, j], oob_preds[mask_valid, j], self.oob_metric
                )

        self.oob_predictions_ = oob_preds
        self.oob_counts_ = oob_counts
        self.oob_errors_ = oob_errors  # numpy array: [general, QMIN...QMAX]

        del rng, seeds, X_noID, results
        gc.collect()
        return self
    ...
    def predict(self, X):
        if X.shape[1] == self.estimators_[0].n_features_in_ + 1:
            X = X[:, 1:]
        preds = Parallel(n_jobs=self.n_jobs)(
            delayed(self._predict_single_tree)(est, X) for est in self.estimators_
        )
...
print('General OOB r2:', RFreg.oob_errors_[0])
print('OOB per quantile:', RFreg.oob_errors_[1:])  # [QMIN, Q10, ..., QMAX]
