...
#### sbatch  /gpfs/gibbs/pi/hydro/hydro/scripts/GSI_TS/sc20_extract_grace_morevar_flowarea_sfd.sh
...
EXTRACT=/gpfs/gibbs/pi/hydro/hydro/dataproces/GSI_TS/extract_area
RAM=/dev/shm
...
# time paste -d " " \
# <( gdallocationinfo -valonly -geoloc $TERRA/tmax_acc/$YYYY/tmax_${YYYY}_${MM}_sfd.vrt     < $RAM/x_y_${DA_TE}.txt) \
# <( gdallocationinfo -valonly -geoloc $TERRA/tmax_acc/$YYYY1/tmax_${YYYY1}_${MM1}_sfd.vrt  < $RAM/x_y_${DA_TE}.txt) \
# <( gdallocationinfo -valonly -geoloc $TERRA/tmax_acc/$YYYY2/tmax_${YYYY2}_${MM2}_sfd.vrt  < $RAM/x_y_${DA_TE}.txt) \
# <( gdallocationinfo -valonly -geoloc $TERRA/tmax_acc/$YYYY3/tmax_${YYYY3}_${MM3}_sfd.vrt  < $RAM/x_y_${DA_TE}.txt) >> $RAM/predictors_values_c_${DA_TE}.txt
...
paste -d " " $RAM/stationID_x_y_value_${DA_TE}.txt $RAM/predictors_values_?_${DA_TE}.txt | sed 's/  / /g' > $EXTRACT/stationID_x_y_value_predictors_${DA_TE}_floareaSFD.txt
...
grep -h -v -e ID -e "\-2147483648 " -e "\-9999999 " -e "72300000.000" -e "61202000.000" $EXTRACT/stationID_x_y_value_predictors_????_??_floareaSFD.txt | awk '{ if (NF==105) print }' | sed  's/  / /g'  | sed 's/ *$//'   >>  $EXTRACT/stationID_x_y_valueALL_predictors_floareaSFD.txt
fi 
