...
#### for obs_leaf in 100 500  ; do for obs_split in 100 500 ; do for sample in 0.9  ; do sbatch --export=obs_leaf=$obs_leaf,obs_split=$obs_split,sample=$sample  /gpfs/gibbs/pi/hydro/hydro/scripts/GSI_TS/sc31_modeling_pythonALL_RFrunMainRespRenkOptim_split_IDraster_testSpars_RFunID_SnapCorRFas30_flowred_OOB.sh   ; done; done ; done 
...
class ClusteredRandomForestRegressor:
    def __init__(self, n_estimators=100, 
                 min_samples_leaf=1, min_samples_split=2, max_features='sqrt',
                 max_samples=None, random_state=None, oob_score=True, n_jobs=-1):
        ...
        self.oob_score_ = None           # global variance-weighted R²
        self.oob_scores_targets_ = None  # per-target R²
        self.oob_predictions_ = None

    def fit(self, X, Y, groups):
        
        # X: array (n_samples, n_features) including IDr as first col
        # Y: array (n_samples, n_targets) only targets
        # groups: array (n_samples,) with IDr
        
        self.cluster_ids_ = np.unique(groups)
        rng = np.random.RandomState(self.random_state)

        # Assign trees to clusters
        cluster_assignments = rng.choice(self.cluster_ids_, size=self.n_estimators)
        ...
        for cid in cluster_assignments:
            mask = groups == cid
            X_c, Y_c = X[mask, 1:], Y[mask]  # drop IDr
        ...
        if self.oob_score:
            ...
            self.oob_predictions_ = np.zeros((n_samples, n_targets))
            self.oob_counts_ = np.zeros(n_samples, dtype=int)

            for est, mask in zip(self.estimators_, cluster_masks):
                oob_mask = ~mask  # samples not used by this tree
                if np.any(oob_mask):
                    preds = est.predict(X[oob_mask, 1:])  # drop IDr
                    preds = np.maximum(preds, 0)  # enforce non-negative
                    self.oob_predictions_[oob_mask] += preds
                    self.oob_counts_[oob_mask] += 1

            valid = self.oob_counts_ > 0
            if np.any(valid):
                self.oob_predictions_[valid] /= self.oob_counts_[valid][..., None]

                # Global variance-weighted R²
                self.oob_score_ = r2_score(
                    Y[valid], self.oob_predictions_[valid], multioutput='variance_weighted'
                )
                # Per-target R² (QMIN … QMAX)
                self.oob_scores_targets_ = r2_score(
                    Y[valid], self.oob_predictions_[valid], multioutput='raw_values'
                )
            else:
                self.oob_score_ = None
                self.oob_scores_targets_ = None

        return self

    def predict(self, X):
        preds = Parallel(n_jobs=self.n_jobs)(
            delayed(lambda est: est.predict(X[:, 1:]))(est)
            for est in self.estimators_
        )
        avg_preds = np.mean(preds, axis=0)
        return np.maximum(avg_preds, 0)  # enforce non-negative

RFreg  = ClusteredRandomForestRegressor(
...
RFreg.fit(X_train_np, Y_train_np , groups=X_train_np[:, 0]  )
...
# Get OOB per target (QMIN … QMAX)
oob_scores = model.oob_scores_targets_
if oob_scores is None:
    oob_scores = np.full(Y_train_np.shape[1], np.nan)  # fill NaN if no OOB
...
merge_oob = np.concatenate((initial_array, oob_scores.reshape(1, -1)), axis=1)

# Define the format strings
fmt = ' '.join(['%i'] + ['%.2f'] + ['%i'] + ['%i'] + ['%.2f'] * (merge_r.shape[1] - 4))

# Save the results to separate files
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YscorerN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_2RF.txt', merge_r, delimiter=' ', fmt=fmt)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YscorerhoN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_2RF.txt', merge_rho, delimiter=' ', fmt=fmt)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YscoremaeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_2RF.txt', merge_mae, delimiter=' ', fmt=fmt)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YscorekgeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_2RF.txt', merge_kge, delimiter=' ', fmt=fmt)

fmt = ' '.join(['%i'] + ['%.2f'] + ['%i'] + ['%i'] + ['%.2f'] * (merge_oob.shape[1] - 4))
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YscoreoobN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_2RF.txt', merge_kge, delimiter=' ', fmt=fmt)
