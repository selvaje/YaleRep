...
#SBATCH -n 1 -c 16  -N 1
#SBATCH -t 24:00:00 
#SBATCH -o /vast/palmer/scratch/sbsc/ga254/stdout/sc31_modeling_python_GLOBAL_OPT2.sh.%A_%a.out
#SBATCH -e /vast/palmer/scratch/sbsc/ga254/stderr/sc31_modeling_python_GLOBAL_OPT2.sh.%A_%a.err
#SBATCH --job-name=sc31_SnapCorGlobalOpt.sh
#SBATCH --array=500
#SBATCH --mem=100G
...
print('='*80)
print('OPTIMIZED GLOBAL MODEL (USING BUILT-IN LAG STRUCTURE)')
print('='*80)
print(f'Config: N_EST={N_EST_I}, leaf={obs_leaf_i}, depth={depth_i}')

print('\nLAG FEATURE STRATEGY:')
print('  ✓ NO manual lag creation needed!')
print('  ✓ Use existing ppt0-3, tmin0-3, tmax0-3, swe0-3, soil0-3')
print('  ✓ Each dynamic var = 4 lags (current + 3 months history)')
print('  ✓ 3-month lookback captures seasonal patterns')
print('  ✓ Decorrelate static features (redundant information)')
print('  ✓ Add spatial (Xcoord, Ycoord) + temporal (MM)')
...
print('\n' + '='*80)
print('FEATURE ENGINEERING (SIMPLIFIED - NO LAG CREATION)')
print('='*80)
...
print(f'    - ppt0-3: 4 precipitation lags')
print(f'    - tmin0-3: 4 temperature min lags')
print(f'    - soil0-3: 4 soil moisture lags')
print(f'  Spatial: 2 features (Xcoord, Ycoord)')
print(f'  Temporal: 1 feature (MM - month)')

final_features = kept_static + dynamic_vars_with_built_in_lags + ['MM', 'Xcoord', 'Ycoord']
...
print(f'  Breakdown: {len(kept_static)} + {len(dynamic_vars_with_built_in_lags)} + 2 + 1')

print('\n' + '='*80)
print('TRAINING GLOBAL ExtraTreesRegressor')
print('='*80)

print(f'Training on {X_train_np.shape[0]} observations, {len(final_features)} features')
t_train = time.time()
...
print(f'\n✓ LAG STRUCTURE IMPORTANCE:')
for lag in [0, 1, 2, 3]:
    lag_features = [f for f in imp_ser.index if f.endswith(f'{lag}')]
    if lag_features:
        avg_imp = imp_ser[lag_features].mean()
        print(f'  Lag-{lag} (t-{lag}): avg importance={avg_imp:.4f} ({len(lag_features)} features)')

imp_ser.to_csv(f'../predict_importance_red/stationID_x_y_valueALL_predictors_XimportanceN{N_EST_S}_GLOBAL_OPTIMIZED.txt', index=True, sep=' ', header=False)
...
fmt_p = '%.2f ' * 11
np.savetxt(f'../predict_prediction_red/stationID_x_y_valueALL_predictors_YpredictTrainN{N_EST_S}_GLOBAL_OPT.txt', Y_tr_sort, delimiter=' ', fmt=fmt_p.strip(), header='QMIN Q10 Q20 Q30 Q40 Q50 Q60 Q70 Q80 Q90 QMAX', comments='')
np.savetxt(f'../predict_prediction_red/stationID_x_y_valueALL_predictors_YpredictTestN{N_EST_S}_GLOBAL_OPT.txt', Y_ts_sort, delimiter=' ', fmt=fmt_p.strip(), header='QMIN Q10 Q20 Q30 Q40 Q50 Q60 Q70 Q80 Q90 QMAX', comments='')

print(f'\n✓ ✓ ✓ OPTIMIZED GLOBAL MODEL COMPLETE ✓ ✓ ✓')
print(f'✓ Strategy: NO manual lag creation (use built-in ppt0-3, etc)')
print(f'✓ Features: {len(kept_static)} static + 20 dynamic (with lags) + 2 spatial + 1 temporal = {len(final_features)} total')
print(f'✓ Lookback: 3 months (captures seasonal patterns)')
print(f'✓ Training: {train_time:.1f}s (very fast!)')
print(f'✓ Inference: Direct prediction (no clustering)')
print(f'✓ Output: 11 quantiles per location')
print(f'✓ Production ready: Simple, fast, easy to maintain')
