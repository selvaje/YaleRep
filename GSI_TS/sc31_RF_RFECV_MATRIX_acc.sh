...
Y = pd.read_csv('stationID_x_y_valueALL_predictors_Y11_floredSFD.txt', header=0, sep='\s+', dtype=dtypes_Y, engine='c', low_memory=False)
X = pd.read_csv('stationID_x_y_valueALL_predictors_X11_floredSFD.txt', header=0, sep='\s+', usecols=lambda col: col in include_variables, dtype=dtypes_X, engine='c', low_memory=False)
...
########################################################################
## Targets for MODELING (NO LOG): q = Q / accumulation
########################################################################
...
Yarea_train = (Qtr.div(acc_tr, axis=0)).fillna(0).astype('float32')
Yarea_test = (Qte.div(acc_te, axis=0)).fillna(0).astype('float32')

Yarea_train_np = Yarea_train.to_numpy()
Yarea_test_np = Yarea_test.to_numpy()
...
rank_out = rf'../predict_score_red/RFECV_static_ranking_N{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample.txt'
...
curve_out = rf'../predict_score_red/RFECV_static_curve_N{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample.txt'
curve_df.to_csv(curve_out, sep=' ', index=False)

########################################################################
## 5) Build global RF model (ExtraTrees) with q = Q/accumulation for static + dynamic_area (ET_SD)
########################################################################
...
ET_SD.fit(X_train_SD, Yarea_train_np)
print(f'OOB r2 ExtraTreesRegressor_SD : {ET_SD.oob_score_}')

########################################################################
## 6) Errors matrix using model ET_SD (static + dynamic_area)
########################################################################
Yarea_test_pred_nosort = ET_SD.predict(X_test_SD)
Yarea_train_pred_nosort = ET_SD.predict(X_train_SD)

assert Yarea_train_pred_nosort.shape[0] == X_train.shape[0] == Y_train.shape[0]
assert Yarea_test_pred_nosort.shape[0] == X_test.shape[0] == Y_test.shape[0]
...
# A) Q/accumulation space (YAscore*)
train_A = compute_error_pack(Yarea_train_np, Yarea_train_pred_nosort)
test_A = compute_error_pack(Yarea_test_np, Yarea_test_pred_nosort)

merge_r_A = np.concatenate((initial_array, train_A['r_all'], test_A['r_all'], train_A['r_coll'], test_A['r_coll']), axis=1)
merge_rho_A = np.concatenate((initial_array, train_A['rho_all'], test_A['rho_all'], train_A['rho_coll'], test_A['rho_coll']), axis=1)
merge_mae_A = np.concatenate((initial_array, train_A['mae_all'], test_A['mae_all'], train_A['mae_coll'], test_A['mae_coll']), axis=1)
merge_kge_A = np.concatenate((initial_array, train_A['kge_all'], test_A['kge_all'], train_A['kge_coll'], test_A['kge_coll']), axis=1)

fmt_score = ' '.join(['%i'] + ['%.2f'] + ['%i'] + ['%i'] + ['%.2f'] * (merge_r_A.shape[1] - 4))

np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YAscorerN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_r_A, delimiter=' ', fmt=fmt_score)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YAscorerhoN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_rho_A, delimiter=' ', fmt=fmt_score)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YAscoremaeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_mae_A, delimiter=' ', fmt=fmt_score)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YAscorekgeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_kge_A, delimiter=' ', fmt=fmt_score)

print('Wrote AREA-normalized scores: YAscore*')

# B) Q space (YQscore*) back-transform by multiplication
...
Q_train_pred = Yarea_train_pred_nosort.astype('float64') * acc_tr_np
Q_test_pred = Yarea_test_pred_nosort.astype('float64') * acc_te_np
...
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YQscorerN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_r_Q, delimiter=' ', fmt=fmt_score)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YQscorerhoN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_rho_Q, delimiter=' ', fmt=fmt_score)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YQscoremaeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_mae_Q, delimiter=' ', fmt=fmt_score)
np.savetxt(rf'../predict_score_red/stationID_x_y_valueALL_predictors_YQscorekgeN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_GaRFG.txt', merge_kge_Q, delimiter=' ', fmt=fmt_score)

print('Wrote Q scores: YQscore*')
...
np.savetxt(rf'../predict_prediction_red/stationID_x_y_valueALL_predictors_YpredictTrainN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_2RF.txt', Yarea_train_pred_nosort, delimiter=' ', fmt=fmt_pred, header=' '.join(q_cols), comments='')
np.savetxt(rf'../predict_prediction_red/stationID_x_y_valueALL_predictors_YpredictTestN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{depth_s}depth_{sample_s}sample_2RF.txt', Yarea_test_pred_nosort, delimiter=' ', fmt=fmt_pred, header=' '.join(q_cols), comments='')
