...
#SBATCH -n 1 -c 12 -N 1
#SBATCH -t 2:00:00
#SBATCH -o /nfs/roberts/scratch/pi_ga254/ga254/stdout/sc31_LSTM_V11.%J.out        # [#9]
#SBATCH -e /nfs/roberts/scratch/pi_ga254/ga254/stderr/sc31_LSTM_V11.%J.err        # [#9]
#SBATCH --job-name=sc31_LSTM_V11                                                   # [#9]
#SBATCH --mem=40G
...
from sklearn.isotonic import IsotonicRegression as _IR   # [#3]
import torch
...
# [#3] Monotonicity helper — enforces QMIN ≤ Q10 ≤ … ≤ QMAX row-wise
_ir_mono = _IR(increasing=True, out_of_bounds='clip')
def enforce_monotonicity(arr2d):
    """Enforce QMIN ≤ Q10 ≤ … ≤ QMAX row-wise via isotonic regression."""
    x_ord = np.arange(arr2d.shape[1])
    return np.array([_ir_mono.fit_transform(x_ord, row) for row in arr2d])

print("\n" + "="*100)
print("SC31: LSTM V11 - SLIDING WINDOW + LOG1P + MONOTONICITY + ASSERTIONS")  # [#9]
print("="*100)
...
STRIDE  = 1          # [#1] sliding-window stride (1 = every month, increase to sub-sample)
SELECTION_LAYERS = 1
...
print(f"  Sliding-window stride: {STRIDE}")                         # [#1]
...
def check_consecutive_months(df, min_length):
    ...
    for station, group in df.groupby('StationID'):
        ...
        for i in range(1, len(group)):
            ...
            if expected_date == group.loc[i, 'date']:
                current_consecutive += 1
...
valid_stations   = [sid for sid, m in station_months.items() if m >= SEQ_LEN]
invalid_stations = [sid for sid, m in station_months.items() if m <  SEQ_LEN]
...
assert (acc > 0).all(), "accumulation must be strictly positive in all rows"  # [#5]
...
for var in accumulated_vars:
    if var in X.columns:
        X[f'{var}_mean'] = (X[var].astype('float32').values / acc).astype('float32')  # [#5]
...
assert (Y_accumulation > 0).all(), "accumulation must be strictly positive in all rows"  # [#5]

q_cols_specific = []
for q_col in q_cols:
    q_specific_col = f'{q_col}_specific'
    Y[q_specific_col] = (Y[q_col].values / Y_accumulation).astype('float32')  # [#5]
...
def split_station_temporal(station_data, train_years=11, test_years=11):
    ...
    if total_months < required_months:
        return {'station_id': station_id, 'train_idx': np.array([]),
                'test_idx': np.array([]), 'total_months': total_months,
                'skipped': True, 'train_dates': None, 'test_dates': None}
    train_idx = station_data.index[:train_months].values
    test_idx  = station_data.index[train_months:train_months + test_months].values
    train_dates = (station_data.iloc[0]['YYYY'], station_data.iloc[0]['MM'],
                   station_data.iloc[train_months-1]['YYYY'], station_data.iloc[train_months-1]['MM'])
    test_dates  = (station_data.iloc[train_months]['YYYY'], station_data.iloc[train_months]['MM'],
                   station_data.iloc[train_months + test_months - 1]['YYYY'],
                   station_data.iloc[train_months + test_months - 1]['MM'])
    return {'station_id': station_id, 'train_idx': train_idx, 'test_idx': test_idx,
            'total_months': total_months, 'skipped': False,
...
train_indices, test_indices, skipped_stations = [], [], []
...
# [#4] log1p symmetrises skewed specific-discharge distribution; expm1 inverts at inference
...
print("✓ Scaling complete (log1p applied to discharge targets)")  # [#4]

# Build sequences
print("\nBuilding sequences...")

# [#1] Sliding-window sequence builder — stride-controlled, one window per offset per station
def build_sequences(df_meta, X_dyn, X_sta, Y, seq_len=132, stride=1):
    """
    Build LSTM sequences with a sliding window of length seq_len.
    For each station, yields (n_months - seq_len) // stride + 1 sequences.
    # [#2] Y stores period-constant quantiles; any row index in the window yields the same value
    """
    ...
    for station_id, group in df.groupby('StationID'):
        ...
        if n < seq_len:
            continue
        # Slide the window across every valid offset
        for start in range(0, n - seq_len + 1, stride):
            end = start + seq_len          # exclusive
            seq_idx = indices[start:end]
            last    = indices[end - 1]
            X_seq_dyn.append(X_dyn[seq_idx])
            X_seq_sta.append(X_sta[last])
            # [#2] Y stores period-constant quantiles; any row index in the window yields the same value
            Y_last.append(Y[last])
...
print(f"Train sequences: {Xtr_seq_dyn.shape[0]:,} (sliding window, stride={STRIDE})")  # [#1]
print(f"Test sequences:  {Xte_seq_dyn.shape[0]:,} (sliding window, stride={STRIDE})")   # [#1]
...
# Keep true targets in ORIGINAL (non-log) space so metrics are interpretable
...
# =========================================================================
# SEQUENTIAL FORWARD SELECTION (STATIC FEATURES ONLY — LSTM-BASED)
# =========================================================================
...
if USE_SEQUENTIAL_SELECTION and len(static_decorrelated) > 0:
    ...
    selection_history = []   # [#7] list of (feature_name, mae_after_adding)
    ...
    train_dl_baseline = DataLoader(train_ds_baseline, batch_size=SELECTION_BATCH,
                                   shuffle=True,  num_workers=NUM_WORKERS, pin_memory=USE_GPU)
    test_dl_baseline  = DataLoader(test_ds_baseline,  batch_size=SELECTION_BATCH * 2,
                                   shuffle=False, num_workers=NUM_WORKERS, pin_memory=USE_GPU)
    ...
    for epoch in range(SELECTION_EPOCHS):
        for x_dyn, x_sta, y in train_dl_baseline:
            ...
            if USE_MIXED_PRECISION:
                with autocast():
                    loss = criterion(model_baseline(x_dyn, x_sta), y)
    ...
    with torch.no_grad():
        for x_dyn, x_sta, y in test_dl_baseline:
            ...
            if USE_MIXED_PRECISION:
                with autocast():
                    p = model_baseline(x_dyn, x_sta)
            else:
                p = model_baseline(x_dyn, x_sta)
            all_preds_baseline.append(p.cpu().numpy())

    preds_baseline = np.vstack(all_preds_baseline)
    # [#4] invert log1p scaling; [#3] enforce monotonicity
    preds_baseline_unscaled = enforce_monotonicity(
        np.expm1(scaler_y.inverse_transform(preds_baseline))
    )
    ...
    for iteration in range(MAX_STATIC_FEATURES):
        ...
        for candidate in remaining_static:
            ...
            train_ds_c = LSTMDataset(Xtr_seq_dyn, Xtr_seq_sta_current, Ytr_seq)
            test_ds_c  = LSTMDataset(Xte_seq_dyn, Xte_seq_sta_current, Yte_seq)
            train_dl_c = DataLoader(train_ds_c, batch_size=SELECTION_BATCH,
                                    shuffle=True,  num_workers=NUM_WORKERS, pin_memory=USE_GPU)
            test_dl_c  = DataLoader(test_ds_c,  batch_size=SELECTION_BATCH * 2,
                                    shuffle=False, num_workers=NUM_WORKERS, pin_memory=USE_GPU)
            opt_c = torch.optim.Adam(model_candidate.parameters(), lr=SELECTION_LR)

            model_candidate.train()
            for epoch in range(SELECTION_EPOCHS):
                for x_dyn, x_sta, y in train_dl_c:
                    x_dyn, x_sta, y = x_dyn.to(DEVICE), x_sta.to(DEVICE), y.to(DEVICE)
                    opt_c.zero_grad()
                    if USE_MIXED_PRECISION:
                        with autocast():
                            loss = criterion(model_candidate(x_dyn, x_sta), y)
                        scaler_amp.scale(loss).backward()
                        scaler_amp.step(opt_c)
                        scaler_amp.update()
                    else:
                        loss = criterion(model_candidate(x_dyn, x_sta), y)
                        loss.backward()
            ...
            all_preds_c = []
            with torch.no_grad():
                for x_dyn, x_sta, y in test_dl_c:
                    x_dyn, x_sta = x_dyn.to(DEVICE), x_sta.to(DEVICE)
                    if USE_MIXED_PRECISION:
                        with autocast():
                            p = model_candidate(x_dyn, x_sta)
                    else:
                        p = model_candidate(x_dyn, x_sta)
                    all_preds_c.append(p.cpu().numpy())

            preds_c = np.vstack(all_preds_c)
            # [#4] invert log1p; [#3] enforce monotonicity
            preds_c_unscaled = enforce_monotonicity(
                np.expm1(scaler_y.inverse_transform(preds_c))
            )
            mae_c = mean_absolute_error(Yte_true, preds_c_unscaled)
            candidate_results.append((candidate, mae_c))

            del model_candidate, train_ds_c, test_ds_c
            gc.collect()
            if USE_GPU:
        ...
        if best_candidate_mae < best_mae:
            ...
            selection_history.append((best_candidate, best_candidate_mae))  # [#7]
    ...
    print(f"\n{'='*100}")
    print(f"SELECTION COMPLETE — IMPROVEMENT TABLE")
    print(f"{'='*100}")
    print(f"{'Feature':<35} {'MAE':>10} {'Δ MAE':>10} {'Δ %':>8}")
    print("-" * 65)
    print(f"{'[baseline - dynamic only]':<35} {mae_baseline:>10.4f} {'—':>10} {'—':>8}")
    cumulative_mae = mae_baseline
    for feat, feat_mae in selection_history:
        delta     = cumulative_mae - feat_mae
        delta_pct = delta / cumulative_mae * 100
        print(f"  + {feat:<32} {feat_mae:>10.4f} {delta:>+10.4f} {delta_pct:>7.1f}%")
        cumulative_mae = feat_mae
    print("-" * 65)
    print(f"{'[final]':<35} {best_mae:>10.4f} "
          f"{mae_baseline - best_mae:>+10.4f} "
          f"{(mae_baseline - best_mae) / mae_baseline * 100:>7.1f}%")
    print(f"{'='*100}")
...
train_dl_final = DataLoader(train_ds_final, batch_size=FINAL_BATCH,
                            shuffle=True,  num_workers=NUM_WORKERS, pin_memory=USE_GPU)
test_dl_final  = DataLoader(test_ds_final,  batch_size=FINAL_BATCH * 2,
                            shuffle=False, num_workers=NUM_WORKERS, pin_memory=USE_GPU)

optimizer_final     = torch.optim.Adam(model_final.parameters(), lr=FINAL_LR)
scheduler           = torch.optim.lr_scheduler.ReduceLROnPlateau(
                          optimizer_final, mode='min', factor=LR_FACTOR, patience=LR_PATIENCE)
...
for epoch in range(FINAL_EPOCHS):
    ...
    for x_dyn, x_sta, y in train_dl_final:
        ...
        if USE_MIXED_PRECISION:
            with autocast():
                loss = criterion(model_final(x_dyn, x_sta), y)
    ...
    if val_loss < best_val_loss:
        ...
        early_stop_counter = 0
        torch.save(model_final.state_dict(), '../predict_score_red/best_model_v11.pt')
    else:
...
model_final.load_state_dict(torch.load('../predict_score_red/best_model_v11.pt'))
...
with torch.no_grad():
    for x_dyn, x_sta, y in test_dl_final:
        ...
        if USE_MIXED_PRECISION:
            with autocast():
                p = model_final(x_dyn, x_sta)
        else:
            p = model_final(x_dyn, x_sta)
        all_preds_final.append(p.cpu().numpy())

preds_final = np.vstack(all_preds_final)

# [#4] invert log1p scaling back to original specific-discharge space
preds_final_raw  = np.expm1(scaler_y.inverse_transform(preds_final))   # [#3] before isotonic
# [#3] enforce QMIN ≤ Q10 ≤ … ≤ QMAX
preds_final_mono = enforce_monotonicity(preds_final_raw)

# [#3] Monotonicity diagnostics
violations_before = int((np.diff(preds_final_raw,  axis=1) < 0).sum())
violations_after  = int((np.diff(preds_final_mono, axis=1) < 0).sum())
print(f"\n  Monotonicity violations: {violations_before} → {violations_after} (after isotonic)")

preds_final_unscaled = preds_final_mono  # use monotone predictions for all metrics
...
results_df.to_csv('../predict_score_red/lstm_v11_results.txt', sep=' ', index=False)
print(f"\n✓ Saved results to: ../predict_score_red/lstm_v11_results.txt")

with open('../predict_score_red/lstm_v11_selected_features.txt', 'w') as f:
    f.write(f"# Selected static features ({len(static_final)})\n")
    for feat in static_final:
        f.write(f"{feat}\n")
print(f"✓ Saved selected features to: ../predict_score_red/lstm_v11_selected_features.txt")
