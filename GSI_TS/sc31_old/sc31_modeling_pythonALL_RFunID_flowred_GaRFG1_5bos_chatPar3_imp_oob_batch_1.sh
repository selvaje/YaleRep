...
#### for obs_leaf in 100  ; do for obs_split in 100 ; do for sample in 0.9  ; do sbatch --export=obs_leaf=$obs_leaf,obs_split=$obs_split,sample=$sample  /gpfs/gibbs/pi/hydro/hydro/scripts/GSI_TS/sc31_modeling_pythonALL_RFunID_flowred_GaRFG1_5bos_chatPar3_imp_oob_batch.sh   ; done; done ; done 
...
class GroupAwareRandomForest:
    ...
    def _fit_single_tree(self, tree, X, y, groups, rng):
        ...
        if groups is not None:
            ...
            indices = np.hstack([np.where(groups == g)[0] for g in sampled_groups])
        else:
            indices = rng.choice(n_samples, n_samples, replace=True)

        X_boot, y_boot = X[indices], y[indices]
        groups_boot = groups[indices] if groups is not None else None

        tree.fit(X_boot, y_boot, groups=groups_boot)

        # cleanup
        del X_boot, y_boot, groups_boot, indices
        gc.collect()
    ...
    def predict(self, X):
        ...
        return np.clip(y_pred, 0, None)  # no negatives
