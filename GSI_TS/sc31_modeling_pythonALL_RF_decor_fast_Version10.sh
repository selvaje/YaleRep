...
Y = pd.read_csv(rf'stationID_x_y_valueALL_predictors_0Y11_floredSFD.txt', header=0, sep='\s+', dtype=dtypes_Y, engine='c', low_memory=False)
X = pd.read_csv(rf'stationID_x_y_valueALL_predictors_0X11_floredSFD.txt', header=0, sep='\s+', usecols=lambda col: col in include_variables, dtype=dtypes_X, engine='c', low_memory=False)
...
## ExtraTreesRegressor_SD  static-selected + dinamic variables, oob r2 grouping at station level 
## ExtraTreesRegressor_D  only  dinamic variables, oob r2 grouping at station level 
## ExtraTreesRegressor_S  static-selected only, oob r2 grouping at station level
## oob r2 for ExtraTreesRegressor_SD & ExtraTreesRegressor_D & ExtraTreesRegressor_S
...
def decorrelate_by_spearman_station_level(X_np, groups, col_names, threshold=0.85):
    ...
    if df_g.shape[1] == 0:
        print(f'1) Static station-level decorrelation: start 0 kept 0 dropped 0')
        return []
    ...
    n_kept = len(keep)
    n_start = len(cols)
    print(f'1) Static station-level decorrelation: start {n_start} kept {n_kept} dropped {n_dropped}')
    return keep

def grouped_oob_r2_mean(et_model, Y_np, groups):
...
# 1) decorrelate static at station level
...
selected_static = np.array(static_keep1, dtype=str)
...
# SD model matrices + names (static-selected + dynamic)
...
# D model matrices + names (dynamic only)
X_train_D = X_train_dyn
X_test_D = X_test_dyn
D_names = dynamic_cols

# S model matrices + names (static-selected only)
X_train_S = X_train_static_sel
X_test_S = X_test_static_sel
S_names = selected_static

if X_train_SD.shape[1] == 0:
    raise RuntimeError('No predictors selected for SD model (static+dynamic).')
if X_train_D.shape[1] == 0:
    raise RuntimeError('No dynamic predictors found for D model.')
if X_train_S.shape[1] == 0:
    raise RuntimeError('No static predictors found for S model.')

# ExtraTreesRegressor_SD (static-selected + dynamic)
...
oob_r2_sd = grouped_oob_r2_mean(ET_SD, Y_train_np, groups_train)
print(f'ExtraTreesRegressor_SD (static-selected + dynamic) grouped OOB R2 mean: {oob_r2_sd:.6f}')

# ExtraTreesRegressor_D (dynamic only)
...
ET_D.fit(X_train_D, Y_train_np)
oob_r2_d = grouped_oob_r2_mean(ET_D, Y_train_np, groups_train)
print(f'ExtraTreesRegressor_D (dynamic only) grouped OOB R2 mean: {oob_r2_d:.6f}')

# ExtraTreesRegressor_S (static-selected only)
...
ET_S.fit(X_train_S, Y_train_np)
oob_r2_s = grouped_oob_r2_mean(ET_S, Y_train_np, groups_train)
print(f'ExtraTreesRegressor_S (static-selected only) grouped OOB R2 mean: {oob_r2_s:.6f}')

# Use SD model downstream
class _WrappedModel:
