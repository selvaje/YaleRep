...
oft-calc -ot Float64   $SC/flow_noDep_tiles_intb1/flow_${zone}${tile}.tif   $RAM/flow_${zone}${tile}_pos.tif   <<EOF
1
#1 0 > #1 -1 * #1 ?
EOF

pksetmask -ot Float64  -co COMPRESS=DEFLATE -co BIGTIFF=YES  -co ZLEVEL=9 -m $SC/flow_noDep_tiles_intb1/flow_${zone}${tile}_msk.tif -msknodata 0 -nodata  -9999999 -i  $RAM/flow_${zone}${tile}_pos.tif -o $SC/flow_noDep_tiles_intb1/flow_${zone}${tile}_pos.tif
