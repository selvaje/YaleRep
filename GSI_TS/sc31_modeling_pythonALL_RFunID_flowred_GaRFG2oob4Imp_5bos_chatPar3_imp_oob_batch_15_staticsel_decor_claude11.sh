...
class GroupAwareMultiOutput(BaseEstimator, RegressorMixin):
    def __init__(self, base_estimator, n_cv_folds=3, n_jobs=1, inner_n_jobs=1,
                 random_state=24, oob_metric='r2', verbose=0, early_stopping_tol=0.005):
    ...
    def fit(self, X, Y, groups=None, X_column_names=None, do_oob_cv=True):
        ...
        if self.verbose > 0:
            print(f'Running GroupKFold OOB CV with {n_splits} splits (early_stopping_tol={self.early_stopping_tol})...')
        
        rng = np.random.RandomState(self.random_state)
        seeds = rng.randint(0, 100000, size=n_splits)
        
        prev_oob_r2 = None
        folds_data = list(enumerate(gkf.split(X, y=Y, groups=self._groups)))
        
        for fold_idx, (trn, tst) in folds_data:
            try:
                est = self.base_estimator(
                    random_state=int(seeds[fold_idx]),
                    n_jobs=self.inner_n_jobs
                )
                est.fit(X[trn], Y[trn])
                preds = est.predict(X[tst])
                oob_preds[tst] = preds
                
                if self.verbose > 1:
                    print(f'  Fold {fold_idx+1}/{n_splits} completed')
                
                # Early stopping: check if OOB improvement is below tolerance
                valid_mask = ~np.isnan(oob_preds).any(axis=1)
                if valid_mask.sum() >= 2:
                    current_r2 = np.nanmean([
                        r2_score(Y[valid_mask, i], oob_preds[valid_mask, i])
                        for i in range(n_targets)
                    ])
                    if self.verbose > 0:
                        improvement = current_r2 - prev_oob_r2 if prev_oob_r2 is not None else float('nan')
                        print(f'  Fold {fold_idx+1} OOB R²={current_r2:.4f} (improvement={improvement:.4f})')
                    if (self.early_stopping_tol is not None and prev_oob_r2 is not None
                            and (current_r2 - prev_oob_r2) < self.early_stopping_tol):
                        if self.verbose > 0:
                            print(f'  Early stopping at fold {fold_idx+1}: improvement < tol {self.early_stopping_tol}')
                        break
                    prev_oob_r2 = current_r2
...
# ✓ STRATIFIED GROUP SAMPLE FOR FEATURE SELECTION (preserve group distribution)
sample_size = min(500000, len(X_train_np))
rng_sample = np.random.RandomState(24)
unique_groups, group_counts = np.unique(groups_train, return_counts=True)
group_fractions = group_counts / group_counts.sum()
target_per_group = np.maximum(1, (group_fractions * sample_size).astype(int))
# Adjust to hit sample_size exactly
overshoot = target_per_group.sum() - sample_size
if overshoot > 0:
    target_per_group[np.argsort(-group_counts)[:overshoot]] -= 1
sample_idx_list = []
for g, n in zip(unique_groups, target_per_group):
    g_idx = np.where(groups_train == g)[0]
    chosen = rng_sample.choice(g_idx, min(n, len(g_idx)), replace=False)
    sample_idx_list.append(chosen)
sample_idx = np.concatenate(sample_idx_list)
rng_sample.shuffle(sample_idx)
...
print(f'Stratified group sampling: {sample_size} rows from {len(unique_groups)} groups for feature selection...')
print(f'Sampled data retains full temporal variance (no monthly aggregation)')

# ✓ Decorrelate static features only (dynamic variables skipped – they are independent by design)
static_indices = [i for i, col in enumerate(all_cols) if col in static_present]
X_static_sample = X_sample[:, static_indices]
static_dec_df = decorrelate_group_fast(pd.DataFrame(X_static_sample, columns=static_present), 'Static', threshold=0.65, verbose=True)
...
if len(static_decorr) == 0:
    ...
    # ✓ Feature importance pre-filtering before RFECV (keep top 80% by importance)
    print(f'\nPre-filtering static features with ExtraTrees importance...')
    pre_filter_est = ExtraTreesRegressor(
        n_estimators=50, max_depth=10, n_jobs=16,
        random_state=24, min_samples_leaf=5
    )
    pre_filter_est.fit(X_static_sample_decorr, Y_sample)
    imp_vals = pre_filter_est.feature_importances_
    imp_threshold = np.percentile(imp_vals, 20)  # keep top 80%
    keep_mask = imp_vals >= imp_threshold
    X_static_sample_decorr = X_static_sample_decorr[:, keep_mask]
    static_decorr_filtered = [c for c, k in zip(static_decorr, keep_mask) if k]
    print(f'  Pre-filter: {len(static_decorr)} → {len(static_decorr_filtered)} static features (kept top 80% by importance)')
    del pre_filter_est
    gc.collect()
    ...
    step_size = max(3, int(len(static_decorr_filtered) * 0.30))
    min_features = max(5, int(len(static_decorr_filtered) * 0.35))
    ...
    selected_static_names = np.array(static_decorr_filtered)[support_mask_static].tolist()
    
    rfecv_results_df = pd.DataFrame({
        'Feature': static_decorr_filtered,
    ...
    print(f'\n✓ Selected {len(selected_static_names)}/{len(static_decorr_filtered)} static features by RFECV (from {len(static_decorr)} after decorrelation)')
