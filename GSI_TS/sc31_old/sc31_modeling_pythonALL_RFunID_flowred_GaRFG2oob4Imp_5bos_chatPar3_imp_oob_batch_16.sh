...
class DecorrelatedMultiOutputRF:
    def __init__(self, n_estimators=200, n_jobs=-1, random_state=24, oob_metric='r2', corr_thresh=0.95, **kwargs):
        ...
        self.kwargs = kwargs
        self.original_column_names = None  # To hold original columns

    def decorrelate(self, X, column_names):
        df = pd.DataFrame(X, columns=column_names)
        # corr_mat = df.corr().abs()
        corr_mat = df.corr(method='spearman').abs()  # Use Spearman's rho here
        upper = corr_mat.where(np.triu(np.ones(corr_mat.shape), k=1).astype(bool))
        drop_cols = [c for c in upper.columns if any(upper[c] > self.corr_thresh)]
    ...
    def _fit_fold(self, X, Y, train_idx, test_idx, seed, X_column_names):
        # Decorrelate training data for fold
        X_train_fold, kept_cols = self.decorrelate(X[train_idx], column_names=X_column_names)
        X_test_fold = pd.DataFrame(X[test_idx], columns=X_column_names)[kept_cols].to_numpy()

        RFreg = ExtraTreesRegressor(
            n_estimators=self.n_estimators,
            random_state=seed,
            n_jobs=self.n_jobs,
            **self.kwargs
        )
        RFreg.fit(X_train_fold, Y[train_idx])
        preds = RFreg.predict(X_test_fold)
        imp = pd.Series(RFreg.feature_importances_, index=kept_cols)

        if self.kept_cols_ is None:
            self.kept_cols_ = kept_cols

        return test_idx, preds, imp, RFreg, kept_cols

    def fit(self, X, Y, groups, X_column_names):
        self.original_column_names = X_column_names  # Store original columns
        n_targets = Y.shape[1]
        ...
        self.models_ = []
        for test_idx, preds, imp, model, kept_cols in results:
        ...
        importances_df = pd.concat(importances, axis=1).fillna(0)
        common_kept = importances_df.index.value_counts()
        self.kept_cols_ = common_kept[common_kept > (common_kept.max() // 2)].index.tolist()
        self.feature_importances_ = importances_df.loc[self.kept_cols_].mean(axis=1)
        ...
        # Final model trained on decorrelated features only
        X_final_df = pd.DataFrame(X, columns=X_column_names)[self.kept_cols_]
        final_RF = ExtraTreesRegressor(
            n_estimators=self.n_estimators,
            random_state=self.random_state,
            n_jobs=self.n_jobs,
            **self.kwargs
        )
        final_RF.fit(X_final_df, Y)
        self.models_.append(final_RF)
        return self

    def predict(self, X):
        if not self.models_ or self.kept_cols_ is None:
            raise ValueError('Model not fitted yet.')
        X_df_all = pd.DataFrame(X, columns=self.original_column_names)
        X_df = X_df_all[self.kept_cols_]
        return self.models_[0].predict(X_df)

#### set sperman to 0.6 wit runing ?2 
RFreg = DecorrelatedMultiOutputRF(
    corr_thresh=0.50,
