...
# **FIX: DEFINE all_cols IMMEDIATELY**
all_cols = X_train_column_names.astype(str)

# **FIX: NOW USE all_cols to find static/dynamic present**
...
if len(static_present) == 0:
    ...
    # Build station-level aggregates (ONE row per station)
    station_df_full = pd.DataFrame(X_train_np, columns=all_cols)
    station_df_full['IDr'] = groups_train
    
    # Get ONE representative row per station for static features
    ...
    print(f'Speedup factor: ~{int(X_train_np.shape[0] / station_summary.shape[0])}x\n')
    
    # Apply decorrelation on station-level (FAST!)
    ...
    if len(static_decorr) == 0:
        ...
        print(f'\nRunning FAST GroupAwareRFECV on station-level data: {X_static_station.shape}')
        ...
        selector_fast = GroupAwareRFECV(
            ...
            step=max(8, int(len(static_decorr) * 0.50)),
        ...
        station_groups = np.arange(len(station_summary), dtype=np.int32)
        selector_fast.fit(
            X_static_station,
