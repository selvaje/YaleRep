...
class GroupAwareDecisionTree(DecisionTreeRegressor):
    ## DecisionTreeRegressor wrapper to allow groups argument and enforce non-negative predictions
    def fit(self, X, y, groups=None, sample_weight=None, check_input=True):
        self.groups_ = groups  # stored (not used in splitting)
        return super().fit(X, y, sample_weight=sample_weight, check_input=check_input)

    def predict(self, X):
        y_pred = super().predict(X)
        return np.clip(y_pred, 0, None)  # ensure non-negative predictions

# # -------------------------------
# # Group-Aware Random Forest (group used only for OOB)
# # -------------------------------
class GroupAwareRandomForest2:
    
    # Modified version:
    # - Bootstrapping samples freely across all observations (ignores group)
    # - OOB evaluation is group-aware: each tree’s OOB only uses groups (IDr)
    #   that were completely unseen by that tree.
    # - Parallelized fit and predict.
    # - Memory optimized.
    ...
    def __init__(self,
                 ...
                 min_samples_leaf=1,
                 max_features='sqrt',
                 bootstrap_frac=1.0,  # fraction of all rows for bootstrapping
                 n_jobs=-1,
                 random_state=None,
                 oob_metric='r2',
                 gc_interval=50):
    ...
    # --------------------------
    # Fit a single tree (bootstrap across all samples)
    # --------------------------

def _fit_single_tree(self, X, y, groups, rng, tree_idx):
    
    # Fit a single DecisionTreeRegressor using within-group subsampling.
    # Each tree sees all groups but only a random subset of their temporal records.
    
    n_samples = X.shape[0]
    unique_groups = np.unique(groups)

    # Collect sampled row indices from all groups
    train_idx = []
    for g in unique_groups:
        g_idx = np.where(groups == g)[0]

        # Sample only a fraction of this group’s records (temporal subsampling)
        n_group_samples = max(1, int(len(g_idx) * self.bootstrap_frac))
    ...
    # Fit the tree — note we pass groups only for potential consistency
    tree.fit(X_boot, y_boot, groups=groups[train_idx] if groups is not None else None)

    # Identify groups that this tree has seen
    seen_groups = np.unique(groups[train_idx]) if groups is not None else None

    # Cleanup memory periodically
    del X_boot, y_boot, train_idx
    if tree_idx % self.gc_interval == 0:
        gc.collect()

    return tree, seen_groups, oob_idx
    ...
    def fit(self, X, y, groups=None):
        ...
        results = Parallel(n_jobs=self.n_jobs, require='sharedmem')(
            delayed(self._fit_single_tree)(X, y, groups, rng, i)
            for i in range(self.n_estimators)
        )

        self.estimators_, seen_groups_list, all_oob_idx = zip(*results)

        # ---------------------
        # OOB evaluation (group-aware)
        # ---------------------
        if self.oob_metric is not None and groups is not None:
            n_samples, n_outputs = y.shape
            preds_sum = np.zeros_like(y, dtype=float)
            preds_count = np.zeros_like(y, dtype=float)
            unique_groups = np.unique(groups)

            for tree, seen_groups in zip(self.estimators_, seen_groups_list):
                if seen_groups is None or len(seen_groups) == 0:
                    continue
                unseen_groups = np.setdiff1d(unique_groups, seen_groups)
                if unseen_groups.size == 0:
                    continue
                mask = np.isin(groups, unseen_groups)
                if not np.any(mask):
                    continue

                preds = tree.predict(X[mask])
                if preds.ndim == 1:
                    preds = preds[:, None]

                preds_sum[mask] += preds
                preds_count[mask] += 1

            # Aggregate OOB
            mask_any = np.any(preds_count > 0, axis=1)
            if np.any(mask_any):
                oob_preds = preds_sum[mask_any] / preds_count[mask_any]
                if self.oob_metric == 'r2':
                    global_r2 = r2_score(y[mask_any], oob_preds, multioutput='uniform_average')
                    per_output_r2 = [r2_score(y[mask_any, j], oob_preds[:, j]) for j in range(y.shape[1])]
        ...
        # Variable importance
        # ---------------------
        importances = np.array([tree.feature_importances_ for tree in self.estimators_])
    ...
    def predict(self, X):
        X = np.asarray(X)
        all_preds = Parallel(n_jobs=self.n_jobs, require='sharedmem')(
...
RFreg = GroupAwareRandomForest2(
    ...
    max_features='sqrt',
    max_depth=12,                   # or lower (10–12) to reduce overfit
    min_samples_leaf=obs_leaf_i,
    min_samples_split=obs_split_i,
    bootstrap_frac=0.9,             # now samples across all groups
    random_state=42,
    n_jobs=-1,
    oob_metric='r2',
    gc_interval=50
