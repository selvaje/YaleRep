...
##### sbatch /vast/palmer/home.grace/ga254/scripts/ONCHO/sc05_downloadPoint_RFmodel.sh
...
mod.rf = ranger(pa ~ ., table , importance="impurity" , write.forest=TRUE) 
print(mod.rf)   
imp=as.data.frame(importance(mod.rf))  
imp.s = imp[order(imp$"importance(mod.rf)",decreasing=TRUE), , drop = FALSE] 
...
print(var)
raster  = rast (Sys.glob(paste0("../input/*/",var,".tif")  ) , extent=e )
RNoData = NoData[NoData$V1 == var , 2 ]
NAflag(raster) = RNoData
...
e <- ext(2.5, 2.6, 4.5, 4.6)
env <- crop(env, e)

env

pred.rf.resp  <- terra::predict(env,mod.rf,  fun=predict , na.rm=TRUE, cores=4 ,  cpkgs="randomForest") 
pred.rf.resp
writeStart(pred.rf.resp,"selVar_mod_rf.tif","GTiff", overwrite=TRUE)
