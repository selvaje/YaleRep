...
def filter_static_by_response_spearman_obs_level_keep_high(X_np, Y_np, col_names, threshold=0.85):
    if len(col_names) == 0:
        print(f'2) Response-vs-static filter kept: 0 of 0')
        return []
    ...
    print(f'2) Response-vs-static filter kept: {len(keep)} of {len(col_names)}')
    print(f'\\n2) Correlation values for all {len(col_names)} static variables after step 1:')
    for cname in col_names:
        print(f'   {cname}: {corr_values[cname]:.4f}')
    return keep
...
# 2) from selected ones in (1), keep ONLY static too correlated with response
...
static_keep2 = filter_static_by_response_spearman_obs_level_keep_high(
