...
#  head -1  stationID_x_y_valueALL_predictors_Y.txt                       > stationID_x_y_valueALL_predictors_Ynoh.txt
#  head -1  stationID_x_y_valueALL_predictors_X.txt                       > stationID_x_y_valueALL_predictors_Xnoh.txt 
#  awk '{if($1<900000) print }' stationID_x_y_valueALL_predictors_Y.txt  >> stationID_x_y_valueALL_predictors_Ynoh.txt
#  awk '{if($1<900000) print }' stationID_x_y_valueALL_predictors_X.txt  >> stationID_x_y_valueALL_predictors_Xnoh.txt 
...
class BoundedGroupAwareRandomForest(RandomForestRegressor, RegressorMixin):
    def __init__(self, *args, n_estimators=100, min_samples_leaf=1, min_samples_split=2, max_samples=None, **kwargs):
        ...
        self.oob_score_ = None  # Initialize OOB score attribute
    ...
    def fit(self, X, Y):
        ...
        # Initialize arrays to store OOB predictions and counts
        if self.oob_score and self.bootstrap:
            oob_predictions = np.zeros_like(Y, dtype=np.float32)
            oob_counts = np.zeros(Y.shape[0], dtype=np.int32)
            oob_indices = np.arange(X.shape[0])

        def train_tree(boot_groups, tree_idx):
            # Select bootstrap sample based on groups
        ...
        # Compute OOB score (R² or MSE) if enabled
        if self.oob_score and self.bootstrap:
            # Avoid division by zero for samples not in any OOB set
            valid_oob = oob_counts > 0
            oob_predictions[valid_oob] /= oob_counts[valid_oob][:, None]
            
            # Calculate R² score for each quantile
            oob_r2_scores = []
            for i in range(Y.shape[1]):  # Iterate over quantiles
                y_true = Y[valid_oob, i]
                y_pred = oob_predictions[valid_oob, i]
                if len(y_true) > 0:
                    r2 = r2_score(y_true, y_pred)
                    oob_r2_scores.append(r2)
                else:
                    oob_r2_scores.append(np.nan)  # Handle case with no OOB samples
            self.oob_score_ = np.nanmean(oob_r2_scores)  # Average R² across quantiles
...
# Output OOB score
if RFreg.oob_score:
    oob_r2 = RFreg.oob_score_
    oob_error = 1 - oob_r2  # Simple proxy for error
...
# Save OOB score to file
if RFreg.oob_score:
    oob_array = np.array([[N_EST_I, sample_f, obs_split_i, obs_leaf_i, oob_r2, oob_error]])
    fmt = '%i %.2f %i %i %.4f %.4f'
    np.savetxt(
        rf'../predict_score_red/stationID_x_y_valueALL_predictors_YscoreoobN{N_EST_S}_{obs_leaf_s}leaf_{obs_split_s}split_{sample_s}sample_2RF.txt',
        oob_array,
        delimiter=' ',
        fmt=fmt,
        header='N_EST sample obs_split obs_leaf oob_r2 oob_error',
        comments=''
    )
