...
#### for obs in 4 5 8 10 15;  do sbatch --export=obs=$obs /gpfs/gibbs/pi/hydro/hydro/scripts/GSI_TS/sc30_modeling_pythonALL_RFrunMainRespRenk_RFunID_OOB_all_multicoreE.sh; done 
#### 2 4 5 8 10 15 
...
export obs=$obs ; export N_EST=$SLURM_ARRAY_TASK_ID
...
#  tail -n 20   *importance* | awk '{  print $1  }' | sort| uniq -c | sort -k 1,1 -g | grep -v -e "=="  -e 67 | tail -25  | awk '{ printf("%s,",$2) }'
## ,'GSWe','dy','AWCtS','GSWr','dyy','tpi','GSWo','slope_curv_max_dw_cel','GRWLl','GSWs','WWP','vrm','swe2','pcurv','stream_diff_up_near','slope_grad_dw_cel','outlet_diff_dw_scatch','swe0','swe1','GRWLc','GRWLd','slope_elv_dw_cel','stream_diff_dw_near','stream_dist_dw_near','stream_dist_proximity'
...
dtypes_X = {col: 'float32' for col in range(7, 42)}
dtypes_X['IDraster'] = 'int32'
dtypes_Y = {col: 'float32' for col in range(7, 18)}
dtypes_Y['IDraster'] = 'int32'

X_train  = pd.read_csv(rf'stationID_x_y_valueALL_predictors_X.txt', header=0, sep=' ', usecols=lambda column: column not in ['ID','YYYY','MM','lon','lat','Xcoord','Ycoord','GSWe','dy','AWCtS','GSWr','dyy','tpi','GSWo','slope_curv_max_dw_cel','GRWLl','GSWs','WWP','vrm','swe2','pcurv','stream_diff_up_near','slope_grad_dw_cel','outlet_diff_dw_scatch','swe0','swe1','GRWLc','GRWLd','slope_elv_dw_cel','stream_diff_dw_near','stream_dist_dw_near','stream_dist_proximity'], dtype=dtypes_X , engine='c' , low_memory=False)
Y_train  = pd.read_csv(rf'./stationID_x_y_valueALL_predictors_Y.txt', header=0, sep=' ', usecols=lambda column: column not in ['ID','YYYY','MM','lon','lat','Xcoord','Ycoord'], dtype=dtypes_Y , engine='c' , low_memory=False )
...
savetxt( rf'./stationID_x_y_valueALL_predictors_YscoreN{N_EST_S}_{obs_s}obs.txt', merge, delimiter=' ',  fmt=fmt  )
